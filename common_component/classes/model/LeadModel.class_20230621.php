...
class LeadModel extends BaseModel {
    ...
    public function getLeadFullDetails($lead_id) {
        ...
        if (!empty($lead_id)) {
            ...
            $sql_query .= ", C.aadhar_no, C.customer_digital_ekyc_flag";
            $sql_query .= " FROM leads LD";
            $sql_query .= " INNER JOIN lead_customer C ON(LD.lead_id = C.customer_lead_id)";
            $sql_query .= " WHERE LD.lead_id=$lead_id AND LD.lead_active=1 AND LD.lead_deleted=0";
    ...
    public function getCityDetails($city_id) {
        ...
        if (!empty($city_id)) {
            ...
            $sql_query = "SELECT m_city_id, m_city_name, m_city_is_sourcing";
...
//            echo $sql;
    ...
    public function getEsignApiLog($lead_id, $method_id) {
...
//        $total_penalty_interest = 0;
        ...
        if (!empty($tempDetails['items']) && count($tempDetails['items']) > 0) {
            ...
            $loan_settled_date = !empty($lead_details["loan_noc_settled_letter_datetime"]) ? $lead_details["loan_noc_settled_letter_datetime"] : 0;
            $loan_closure_date = !empty($lead_details["loan_noc_closed_letter_datetime"]) ? $lead_details["loan_noc_closed_letter_datetime"] : 0;
