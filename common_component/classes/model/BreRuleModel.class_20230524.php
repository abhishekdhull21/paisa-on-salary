...
//            echo json_encode($customerResult);
    ...
    public function NOUse_getCustomerBankAccountDetails($lead_id, $cust_bank_id = null) {
    ...
    public function getBureauDetails($lead_id) {
        ...
        $sql = "SELECT lead_id, api1_response as response FROM tbl_cibil_log WHERE lead_id=$lead_id";
        ...
        if (!empty($tempDetails['items']) && count($tempDetails['items']) > 0) {
            ...
            $tempApiResponseXml = $tempDetails['items'][0]['response'];
            ...
            $return_array["status"] = 1;
            $return_array['bureau_data']["score"] = $responseData['SCORES']['SCORE']['SCORE-VALUE'];
            $return_array['bureau_data']["over_due_accounts"] = $responseData['ACCOUNTS-SUMMARY']['PRIMARY-ACCOUNTS-SUMMARY']['PRIMARY-OVERDUE-NUMBER-OF-ACCOUNTS'];
