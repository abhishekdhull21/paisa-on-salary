...
//    }
    ...
    try {
        ...
        $app_data = !empty($LeadDetails['app_data']) ? $LeadDetails['app_data'] : "";
        $lead_status_id = !empty($app_data['lead_status_id']) ? $app_data['lead_status_id'] : "";
        $aadhar_no = !empty($app_data['aadhar_no']) ? trim(strtoupper($app_data['aadhar_no'])) : "";
        ...
        if (empty($aadhaar_document_id)) {
            throw new Exception("Missing aadhaar document id.");
        }

        if (empty($aadhaar_document_type_id)) {
            throw new Exception("Missing aadhaar document type id.");
        }

        if (!in_array($aadhaar_document_type_id, array(1, 2, 3))) {
            throw new Exception("Aadhaar document type id is out of range.");
        }

        $aadhaarDocsDetails = $leadModelObj->getDocumentDetails($lead_id, $aadhaar_document_type_id, $aadhaar_document_id);

        if ($aadhaarDocsDetails['status'] != 1) {
            throw new Exception("Please upload the Aadhaar document.");
        }

        $aadhaar_doc_data = !empty($aadhaarDocsDetails['doc_data']) ? $aadhaarDocsDetails['doc_data'] : "";
        $aadhaar_document_id = !empty($aadhaar_doc_data['docs_id']) ? $aadhaar_doc_data['docs_id'] : "";
        $aadhaar_doc_masked = !empty($aadhaar_doc_data['docs_aadhaar_masked']) ? $aadhaar_doc_data['docs_aadhaar_masked'] : "";

        if (empty($aadhaar_document_id)) {
            throw new Exception("Please upload the Aadhaar document..");
        }

        if ($aadhaar_doc_masked == 1) {
            throw new Exception("Document already masked.");
        }

        if (!file_exists(COMP_DOC_PATH . $aadhaar_doc_data['file'])) {
        ...
        if ($api_call_flag) {
            ...
            $file_url = COMP_DOC_URL . $aadhaar_doc_data['file'];
    ...
    if ($apiStatusId == 1) {
        $lead_remarks = "AADHAAR MASKED API CALL(Success) | AADHAAR NO : $aadhar_no | Customer Name : " . $customer_full_name;
        $file_name = "MASK_" . $aadhaar_doc_data['file'];

        $get_file_data = file_get_contents($masked_document_url);

        if ($get_file_data != false) {

            $doc_flag = file_put_contents(COMP_DOC_PATH . $file_name, $get_file_data);

            if ($doc_flag) {

                $docs_update_array = array();
                $docs_update_array['file'] = $file_name;
                $docs_update_array['docs_aadhaar_masked'] = 1;

                $leadModelObj->updateDocsTable($lead_id, $aadhaar_document_id, $docs_update_array);
