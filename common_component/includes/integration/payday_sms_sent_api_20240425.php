...
function routemobile_sms_sent_api_call($sms_type_id, $lead_id = 0, $request_array = array()) {
    ...
    $repayment_amount = "";
    $repayment_date = "";
    $repayment_link = "https://www.surya.com/repay-loan";
...
//        }
        if ($sms_type_id == 1) {//Mobile OTP Sms New
            ...
            $input_message = "Dear " . $cust_name . ", We hope this message finds you well. Did you know that we offer affordable \n and flexible loan options to help you achieve your financial goals? Whether \n you're looking to fund a home renovation, consolidate debt, or make a major \n purchase, our loan options can help you get there.
                            ...
                            Visit our website.\n
                            Bharat Loan (DEVMUNI FINANCE)";
        } else if ($sms_type_id == 18) {//Collection Pending SMS
            if (empty($cust_name) || empty($loan_no) || empty($repayment_link_generate)) {
                throw new Exception('Customer Name or Loan no or Repayment Link is blank.');
            }
            $template_id = "1207167973066040871";
            $source = "BLCOLX";
            $input_message = "Hi " . $cust_name . ",  We noticed that your Loan No. " . $loan_no . " repayment is overdue.  Please make your payment as soon as possible to avoid any further fees or penalties.  Repayment Link " . $repayment_link_generate . ". Bharat Loan (DEVMUNI FINANCE)";
        }
        else if ($sms_type_id == 19) {//Collection Pending SMS
            if (empty($cust_name)) {
                throw new Exception('Customer Name or Upload document link is blank.');
            }
            $template_id = "1207167973066040871";
            $source = "BLCOLX";
            $input_message = "Hi " . $cust_name . ", Please upload documents click here link ".$upload_doc_link." Bharat Loan (DEVMUNI FINANCE)";
        }
