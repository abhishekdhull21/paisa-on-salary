...
function bank_analysis_doc_upload_api($lead_id, $doc_id, $request_array = array()) {
    ...
    try {
        ...
        if (empty($filename)) {
            ...
            if (!in_array(strtoupper($documentDetails['docs_type']), ['BANK_STATEMENT'])) {
        ...
        // $temp_file = file_put_contents(COMP_DOC_PATH.$filename, file_get_contents(COMP_DOC_URL . $filename));
        // $filePath = $document_path . $filename;
        $filePath = COMP_DOC_PATH.$filename;
       ...
       //  print_r($return_document_id); die;
        ...
        //     'metadata' => '{"password":"' . $pdf_password . '"}'
        // ];
    //   ,"bank":"","name":""
        ...
        $file_path = COMP_DOC_PATH.$filename;
        $dataset = file_get_contents($filePath);
        $file_data = array(
        'file' => new CURLFile($file_path), // Initialize CURLFile object with the file path
        'metadata' => $metadata, // Metadata
        'Filename' => basename($file_path) // Set the filename
        );
        ...
        curl_setopt_array($curl, array(
            ...
            CURLOPT_POSTFIELDS => '{
            "file":"'.base64_encode($dataset).'"
        }',
            CURLOPT_HTTPHEADER => array(
                'auth-Token: '.$apiToken,
                'Metadata: {"password": "'.$pdf_password.'","bank":"Other", "name":"Loan A.pplication Number","productType" : "Salaried"}',
                'Filename: '.$filename,
        ...
        curl_close($curl);
        //echo $response;
        //$apiResponseJson = json_decode($response);
        //echo $apiResponseJson->docId;
        }
        ...
        if (curl_errno($curl) && !$hardcode_response) {
            ...
            $newDocId=$apiResponseData['docId'];
            $status=$apiResponseData['status'];

            // print_r($newDocId); die;

            if (!empty($apiResponseData)) {

                $apiResponseData = common_trim_data_array($apiResponseData);

                if (!empty($status) && in_array(strtolower($status), array("submitted", "processed", "in progress", "downloaded"))) {//Deleted need to ask from provider
                    if (!empty($newDocId)) {

                        $return_document_id = $newDocId;
                        $apiStatusId = 1;

                        if (in_array(strtolower($status), array("processed", "downloaded"))) {
