...
//        $aadhar_no_last_4_digit = substr($aadhar_no, 8, 4);
        ...
        $base64String_file = COMP_DOC_URL . $sanction_letter_file_name;

        $base64String = base64_encode(file_get_contents(COMP_DOC_URL . $sanction_letter_file_name));

        $apiRequestJson = '{
                            "base64String":"' . $base64String_file . '",
        ...
        if (!$hardcode_response && curl_errno($curl)) { // CURL Error
            ...
            if (!empty($apiResponseData)) {
                ...
                if (!empty($apiResponseData)) {
                    ...
                    if (isset($apiResponseData['file']) && !empty($apiResponseData['file'])) {

                        $apiResponseData = $apiResponseData['file'];

                        if (!empty($apiResponseData['id']) && !empty($apiResponseData['directURL'])) {
                            $apiStatusId = 1;
                            $directURL = $apiResponseData['directURL'];
                        } else {
    ...
    if ($apiStatusId == 1) {
        $response_array = esign_aadhaar_request_api_call(2, $lead_id, array());
    }
...
//    }
    ...
    $lw_redirect_url = COMP_CRM_URL . 'sanction-esign-response?lead_id=' . $lead_id;
    $lw_callback_url = "";
...
//    }
        ...
        if (!$hardcode_response && curl_errno($curl)) { // CURL Error
            ...
            if (!empty($apiResponseData)) {
                ...
                if (!empty($apiResponseData)) {
                    ...
                    $apiCustomerData = !empty($apiResponseData['dscData']) ? $apiResponseData['dscData'] : "";

                    if (isset($apiResponseData['result']) && !empty($apiResponseData['result'])) {

                        $apiResponseData = $apiResponseData['result'];

                        if (!empty($apiResponseData['esignedFile'])) {
                            $apiStatusId = 1;
                            $aadhaar_esign_document_url = $apiResponseData['esignedFile'];
                        } else if ($apiResponseData['isUsed'] == 0) {
                            throw new ErrorException("Customer did not complete the eSign.");
                        } else {
    ...
    if ($apiStatusId == 1) {
        ...
        if (trim($apiCustomerData['uidLastFourDigits']) == $aadhar_no_last_4_digit && trim($apiCustomerData['yob']) == $customer_dob_year) {

            $lead_remarks .= "<br/>Result : Aadhaar last four digit and dob year match";

            $eSignFileName = "ESIGN_" . $sanction_letter_file_name;
            ...
            $lead_remarks .= "<br/>eSign File Status : failed";
            $lead_remarks .= "<br/>" . $errorMessage;
        } else if (trim($apiCustomerData['yob']) != $customer_dob_year) {
            $apiStatusId = 8;
            $errorMessage = "Aadhaar DOB year does not matched with customer given DOB.[" . $apiCustomerData['yob'] . "]";
            $lead_remarks .= "<br/>eSign File Status : failed";
            $lead_remarks .= "<br/>" . $errorMessage;
        }
    } else {
