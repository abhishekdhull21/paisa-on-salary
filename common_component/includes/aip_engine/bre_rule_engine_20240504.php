...
function bre_quote_engine($lead_id, $request_array = array()) {
    ...
    $return_array = array("status" => 0, "error" => "", "max_loan_amount" => 0, "min_loan_amount" => 0, "max_loan_tenure" => 0, "interest_rate" => 0, "processing_fee" => 0);
    ...
    $return_array = array("status" => $return_status_id, "error" => $errorMessage, "max_loan_amount" => $max_loan_amount,"min_loan_amount" => $min_loan_amount, "max_loan_tenure" => 40, "interest_rate" => 1, "processing_fee" => 10, "eligible_foir_percentage" => $eligible_foir_percentage);
...
//            common_log_writer(6, "$lead_id || cart_response => " . $bank_analysis_data['cart_response']);
            ...
            $bank_analysis_average_balance = trim($bank_analysis_data['data'][0]['camAnalysisData']['averageBalanceLastSixMonth']);
        }
// print_r($bankAnalysisDetails); 
      ...
      //  $blackListedDetails = $breRuleModelObj->checkBlackListedCustomer($lead_id);
        ...
        $pancard_verified_status = $app_data['pancard_verified_status'];
        ...
        $pan_verification_rule_relevant_inputs = ["customer_pancard" => $customer_pancard, "pancard_verify" => (($pancard_verified_status == 1) ? "Yes" : "No")];
        ...
        } else if (!empty($customer_pancard_dob) && !empty($customer_aadhaar_dob) && ($customer_pancard_dob != $customer_aadhaar_dob || $customer_dob != $customer_pancard_dob || $customer_dob != $customer_aadhaar_dob)) {
            $dob_match_rule_system_decision_id = 2;
            $dob_match_rule_manual_decision_id = 2;
            $dob_match_rule_actual_value = ["PAN and AADHAAR DOB Matched" => "No"];
            $dob_match_rule_relevant_inputs = ["customer_dob" => date("d-m-Y", strtotime($customer_dob)), "customer_pancard_dob" => date("d-m-Y", strtotime($customer_pancard_dob)), "customer_aadhaar_dob" => date("d-m-Y", strtotime($customer_aadhaar_dob))];
        } else if ($user_type != "REPEAT") {
        ...
        if (!empty($bank_analysis_account_no) && !empty($bank_account_number) && substr($bank_analysis_account_no, -4, 4) == substr($bank_account_number, -4, 4) && $bank_analysis_account_ifsc_code == $bank_account_ifsc_code) {
