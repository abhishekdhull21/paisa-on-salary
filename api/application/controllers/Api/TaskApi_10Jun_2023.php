...
//                echo "Shubham4";
                ...
                $insertDataLeads = array(
                    ...
                    'qde_consent' => htmlspecialchars($post['checkbox']),
    ...
    public function savePersonalDetails_post() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && $header_validation) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $email_personal = htmlspecialchars($post['email_personal']);
    ...
    public function saveApplicationDetails_post() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && $header_validation) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $lead_id = htmlspecialchars($post['lead_id']);
                $input_state_id = htmlspecialchars($post['state_id']);
                $input_city_id = htmlspecialchars($post['city_id']);
                $pincode = htmlspecialchars($post['pincode']);
                $loan_amount = htmlspecialchars($post['loan_amount']);
                $monthly_income = htmlspecialchars($post['monthly_income']);
                ...
                if (empty($lead_reference_no)) {
                    ...
                    $dataleads = array(
                        ...
                        'loan_amount' => htmlspecialchars($post['loan_amount']),
                        'term_and_condition' => "YES",
                        'obligations' => (!empty($post['obligations']) ? htmlspecialchars($post['obligations']) : 0),
                        'state_id' => $input_state_id,
                        'city_id' => $input_city_id,
                        'pincode' => ($post['pincode'] ? htmlspecialchars($post['pincode']) : ''),
                    );
                    ...
                    $datacustomer = array(
                        ...
                        'state_id' => $input_state_id,
                        'city_id' => $input_city_id,
                        'cr_residence_pincode' => ($post['pincode'] ? htmlspecialchars($post['pincode']) : ''),
                    );
        ...
        try {
            ...
            if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                ...
                if ($this->form_validation->run() == FALSE) {
                    ...
                    $lead_id       = htmlspecialchars($post['lead_id']);
                    $document_id   = htmlspecialchars($post['docs_type']);
                    $password      = htmlspecialchars($post['password']);
                    $ReferenceCode = htmlspecialchars($post['refrence_no']);
    ...
    public function SaveContactEnquiry_post() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && $header_validation) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $city_name = htmlspecialchars($post['city']);
                $getCityId = $this->Tasks->getcustId('master_city', 'm_city_name', $city_name, 'm_city_id');

                $DataContactEnquiry = array(
                    'cust_enquiry_name' => htmlspecialchars(strtoupper($post['name'])),
                    'cust_enquiry_mobile' => htmlspecialchars($post['mobile']),
                    'cust_enquiry_email' => htmlspecialchars(strtoupper($post['email'])),
                    'cust_enquiry_loan_amount' => intval(htmlspecialchars($post['loan_amount'])),
                    'cust_enquiry_city_name' => htmlspecialchars(strtoupper($post['city'])),
                    'cust_enquiry_city_id' => htmlspecialchars($getCityId),
                    "cust_enquiry_data_source_id" => 1,
                    "cust_enquiry_type_id" => 1,
                    "cust_enquiry_ip_address" => htmlspecialchars($post['ip']),
                    "cust_enquiry_geo_coordinates" => htmlspecialchars($post['coordinates']),
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && $header_validation) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $DataContactEnquiry = array(
                    ...
                    'cust_enquiry_mobile' => htmlspecialchars($post['mobile']),
                    'cust_enquiry_email' => htmlspecialchars(strtoupper($post['email'])),
                    'cust_enquiry_remarks' => htmlspecialchars(strtoupper($post['message'])),
                    "cust_enquiry_data_source_id" => 1,
...
//                    exit;
                ...
                if (empty($result)) {
                    ...
                    $maillink = 'https://www.surya.com/subscribe-email-verify' . "/" . $encrypted_id;
                    ...
                    $html = '<!DOCTYPE html>
                                    ...
                                    <tr>
                                    <td width="975" align="center"><img src="https://www.surya.com/public/images/brand_logo.png" style="width:150px;"></td>
                                    </tr>
                                    <tr>
                                    <td style="text-align:center;"><table width="618" border="0" style="text-align:center;padding:20px;background: #fff;">
                                    <tr>
                                    <td width="auto" align="center"><img src="https://www.surya.com/public/emailimages/verification-email.png" style="width:auto;"></td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                    <td align="center">
                                    <a href="https://www.facebook.com/surya-105632195732824" target="_blank">
                                    <img src="https://www.surya.com/public/image/surya-facebook.png" class="socil-t" alt="surya-facebook" style="width:30px;"></a>
    ...
    public function lendingLeadSave_post() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && $header_validation) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $pancard = htmlspecialchars(strtoupper($post['pan']));
                // echo $pancard; exit;
                ...
                $insertDataLeads = array(
                    ...
                    'email' => $email,
                    'otp' => $otp,
                    'alternate_email' => htmlspecialchars($post['email_office']),
                    'pancard' => $pancard,
                    'loan_amount' => htmlspecialchars($post['loan_amount']),
                    'obligations' => ($post['obligations'] ? htmlspecialchars($post['obligations']) : ''),
                    'state_id' => htmlspecialchars($post['state_id']),
                    'city' => htmlspecialchars($post['city']),
                    'pincode' => ($post['pin'] ? htmlspecialchars($post['pin']) : ''),
                    'lead_entry_date' => created_date,
                    'created_on' => created_on,
                    'source' => htmlspecialchars($post['source']),
                    'ip' => htmlspecialchars($post['ip']),
                    ...
                    'coordinates' => ($post['coordinates']) ? htmlspecialchars($post['coordinates']) : "",
                    'utm_source' => ($post['utm_source'] ? htmlspecialchars($post['utm_source']) : ''),
                    'utm_campaign' => ($post['utm_campaign'] ? htmlspecialchars($post['utm_campaign']) : ''),
                );
                ...
                $insertLeadsCustomer = array(
                    ...
                    'alternate_email' => htmlspecialchars($post['email_office']),
                    'state_id' => htmlspecialchars($post['state_id']),
                    'current_city' => htmlspecialchars($post['city']),
                    'cr_residence_pincode' => ($post['pin'] ? htmlspecialchars($post['pin']) : ''),
                    'created_date' => created_on
                );
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && $header_validation) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                try {
                    ...
                    $ip_address = htmlspecialchars($post['ip']);
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (!empty($lead_id)) {
                    ...
                    if ($response->num_rows() > 0) {
                        ...
                        $scheduled_datetime = htmlspecialchars($post['schedule_datetime']);
                        $remarks = htmlspecialchars($post['remarks']);
                        $current_datetime = date("Y-m-d H:i:s");
                        $scheduled_datetime_hours = intval(date('His', strtotime($scheduled_datetime)));
