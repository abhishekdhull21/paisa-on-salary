            ...
            if ($this->form_validation->run() == FALSE) {
               ...
               // $mobile = intval($post['mobile']);
                ...
                $loan_amount = !empty($post['loan_amount']) ? doubleval($post['loan_amount']) : "";
                   ...
                   $update_cust_leads = $this->db->where('customer_lead_id', $lead_id)->update('lead_customer', $update_lead_customer);
...
//                    $last_name = $result_cust->sur_name;
    ...
    $header_validation = ($headers['Accept'] == "application/json") && ($token['token_Leads'] == base64_decode($headers['Auth']));
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($responseData && isset($responseData['result'])) {
                        ...
                        $leads_pancard = $this->db->select('*')->where('pancard', $matchpancard)->from('leads')->get();
                                 ...
                                 $update_personal_details = [
                                  ...
                                  if($rejectcase){
                                       ...
                                       $update_data_lead_customer = [
                                            ...
                                            'pancard_ocr_verified_status' => $pancardCif->cif_digital_ekyc_flag,
                                            'pancard_ocr_verified_on' => $pancardCif->cif_digital_ekyc_datetime,
                                            ...
                                            'customer_ekyc_request_ip' => $pancardCif->customer_ekyc_request_ip,
                                            'aadhaar_ocr_verified_status' => $pancardCif->cif_digital_ekyc_flag,
                                            ...
                                            'customer_qualification_id' => $pancardCif->cif_qualification_id,
                                            'aadhaar_ocr_verified_on' => $pancardCif->cif_digital_ekyc_datetime,
                                            ...
                                            'customer_religion_id' => $pancardCif->cif_religion_id,
                                            'customer_spouse_occupation_id' => $pancardCif->cif_spouse_occupation_id,
                                            'customer_marital_status_id' => $pancardCif->cif_marital_status_id,
                                            'customer_spouse_name' => $pancardCif->cif_spouse_name,
   ...
   public function AddRepeatAmount_post() {
        ...
        $loan_amount = ($post['loan_amount']);
         ...
         $query = $this->db->select('lead_id, pancard')->where('lead_id', $lead_id)->from('leads')->get();
         $result = $query->row();
