...
//print_r($last_row); die;
    ...
    function panVerification_post() {
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST' && $header_validation) {
			...
			$this->form_validation->set_rules("panNumber","PAN", 
		...
		$leadRepSql = $this->db->select('*')->where('pancard', $panNumber)->where_in('lead_status_id', [14,16,17])->from('leads')->order_by('lead_id', 'desc')->get();
    ...
    public function updatePersonalDetails_post() {
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST' && $header_validation) {
            ...
            if ($this->form_validation->run() == FALSE) {
				...
				/* 
				$normalcity = [12,2,4,3,5,41,7,48,8,9,15,10,71,11,16,315,157,39,10048,10049,667,68,641];
				$oglcity = [115,34,125,14,49,91,186]; 
				*/
				$normalcity = [348,12,2,4,3,41,7,6,48,8,9,15,10,71,11,16,315,157,39,10048,10049,667,68,641];
				$oglcity = [34,49,91,115,125,186];
				$flagCityCheck = 0;
				$salaryMessage = '';
				$rejectMessage = 'City is not allow to take loan.';
				if (isset($city_id) &&  (in_array($city_id, $normalcity)) && ($monthly_amount >=25000)) {
					$flagCityCheck = 0;					
				}                         
				else if (isset($city_id) &&  (in_array($city_id, $oglcity)) && ($monthly_amount >=50000)) {
					$flagCityCheck = 0;					
				}
				else {
					$flagCityCheck = 1;
					$lead_rejected_reason_id = 30;
					if (isset($city_id) &&  (in_array($city_id, $normalcity)) && ($monthly_amount < 25000)) {
						$lead_rejected_reason_id = 14;
						$rejectMessage = "Customer not eligible for loan due to low salary";
					}
					if (isset($city_id) &&  (in_array($city_id, $oglcity)) && ($monthly_amount < 50000)) {
						$lead_rejected_reason_id = 14;
						$rejectMessage = "Customer not eligible for loan due to low salary";
					}
				}
				...
				if($flagCityCheck == 1){
					$update_personal_details = [
						...
						'stage' => 'S9',
						'status' => 'REJECT',
						'lead_rejected_reason_id' => $lead_rejected_reason_id,
						'updated_on' => date('Y-m-d H:i:s')
					];
					...
					$response = ['Status' => 3, 'Message' => $rejectMessage, 'lead_id' => $lead_id, 'data' => $steps];
					$this->Tasks->insertApplicationLog($lead_id, 9, $rejectMessage); 
				} 
				else {
					$update_personal_details = [                            
						'pincode' => $pincode,
	...
	public function saveCustomerDocument_post() {
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST' && $header_validation) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$image_name = $leadDetails['mobile'] . "_" . $lead_id . "_" . $docs_id;
				if(strtolower($post['ext'])=='pdf'){
