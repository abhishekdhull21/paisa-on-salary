...
//                $purpose_of_loan = !empty($post['purpose_of_loan']) ? strtoupper($post['purpose_of_loan']) : '';
                ...
                $update_lead_data = array(
                    'obligations' => htmlspecialchars($obligations),
...
//                $residence_since .= !empty($post['residence_since_month']) ? "-" . $post['residence_since_month'] : "";
                ...
                $update_data_lead_customer = array(
                    'current_house' => htmlspecialchars($house_flat_no),
                    'current_locality' => htmlspecialchars($locality),
                    'current_residence_since' => htmlspecialchars($residence_since),
                    'state_id' => htmlspecialchars($state_id),
                    'city_id' => htmlspecialchars($city_id),
                    'current_state' => htmlspecialchars($getStateName),
                    'current_city' => htmlspecialchars($getCityName),
                    'cr_residence_pincode' => htmlspecialchars($pincode),
                    'current_landmark' => htmlspecialchars($landmark),
                    'current_residence_type' => htmlspecialchars($residence_type),
                    'current_residing_withfamily' => htmlspecialchars($residence_with_family),
                    'alternate_mobile' => htmlspecialchars($alternate_mobile),
...
//                $office_since_date .= !empty($post['residence_since_month']) ? "-" . $post['residence_since_month'] : "";
                ...
                $customer_emp = array(
                    'employer_name' => htmlspecialchars($emp_name),
                    'emp_pincode' => htmlspecialchars($pincode),
                    'state_id' => htmlspecialchars($state_id),
                    'city_id' => htmlspecialchars($city_id),
                    'emp_house' => htmlspecialchars($shop_no),
                    'emp_shopNo' => htmlspecialchars($shop_no),
                    'emp_street' => htmlspecialchars($locality),
                    'emp_locality' => htmlspecialchars($locality),
                    'emp_landmark' => htmlspecialchars($landmark),
                    'emp_residence_since' => htmlspecialchars($employed_since),
                    'emp_designation' => htmlspecialchars($designation),
                    'emp_department' => htmlspecialchars($department),
                    'emp_employer_type' => htmlspecialchars($emp_type),
                    'emp_website' => htmlspecialchars($website),
                ...
                $update_data_lead_customer = array(
                    'alternate_email' => htmlspecialchars($email),
                    'updated_at' => date('Y-m-d H:i:s')
                );
    ...
    public function qdeAppSaveBankDetails_post() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && $header_validation) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $beneficiary_name = !empty($post['beneficiary_name']) ? trim($post['beneficiary_name']) : "";
                ...
                $customer_banking_data = array(
                    'lead_id' => htmlspecialchars($lead_id),
                    'beneficiary_name' => htmlspecialchars($beneficiary_name),
                    'bank_name' => htmlspecialchars($bank_name),
                    'ifsc_code' => htmlspecialchars($ifsc_code),
                    'branch' => htmlspecialchars($branch),
                    'account' => htmlspecialchars($account),
                    'confirm_account' => htmlspecialchars($confirm_account),
...
//        header('Content-Type: application/json; charset=utf-8');
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {// && $header_validation
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $lead_id = htmlspecialchars($post['lead_id']);
                $docs_id = htmlspecialchars($post['docs_id']);
                $docs_file = htmlspecialchars($post['file']);
                $docs_password = htmlspecialchars($post['password']);
    ...
    public function qdeAppRequiredUploadedDocs_post() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                return json_encode($this->response(['Status' => $status, 'Required_Documents' => $docs_lists, 'Message' => $Message, 'reference_no' => $lead_reference_no], REST_Controller::HTTP_OK));
            }
        } else {
            return json_encode($this->response(['Status' => 0, 'Message' => 'Request Method Post Failed.'], REST_Controller::HTTP_OK));
        }
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && $header_validation) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $reference_details = array(
                    'lcr_lead_id' => htmlspecialchars($post['lead_id']),
                    'lcr_name' => htmlspecialchars(strtoupper($post['ref_name'])),
                    'lcr_relationType' => htmlspecialchars($post['relation_type']),
                    'lcr_mobile' => htmlspecialchars($post['mobile']),
                ...
                $table2 = 'leads';
                $column = 'lead_id';
                $update_id = htmlspecialchars($post['lead_id']);
                $leadData = array(
    ...
    public function qdeAppSaveRegistration_post() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && $header_validation) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $lead_mobile_android_id = !empty($post['fcm_token']) ? $post['fcm_token'] : '';
                $finbox_customer_id = !empty($post['finbox_customer_id']) ? $post['finbox_customer_id'] : '';
//                $lead_mobile_android_id = !empty($post['lead_mobile_android_id']) ? $post['lead_mobile_android_id'] : '';
                ...
                $insertDataLeads = array(
                    ...
                    'pancard' => htmlspecialchars($pancard),
                    'otp' => htmlspecialchars($otp),
                    'email' => $email,
                    'company_id' => 1,
                    'product_id' => 1,
                    'lead_status_id' => htmlspecialchars($lead_status_id),
                    'user_type' => 'NEW',
                    'stage' => 'S1',
                    'status' => 'LEAD-NEW',
                    'source' => htmlspecialchars($data_source_name),
                    'lead_entry_date' => date('Y-m-d'),
                    'lead_data_source_id' => htmlspecialchars($lead_data_source_id),
                    'ip' => $ip,
                    'lead_mobile_android_id' => htmlspecialchars($lead_mobile_android_id),
                    'qde_consent' => 'Y',
                    'term_and_condition' => "YES",
                    'created_on' => date('Y-m-d H:i:s'),
                    'city_id' => htmlspecialchars($city_id),
                    'state_id' => htmlspecialchars($city_state_id),
                ...
                $insertLeadsCustomer = array(
                    'customer_lead_id' => htmlspecialchars($lead_id),
                    'first_name' => htmlspecialchars($first_name),
                    'middle_name' => htmlspecialchars($middle_name),
                    'sur_name' => htmlspecialchars($last_name),
                    'mobile' => htmlspecialchars($mobile),
                    'pancard' => htmlspecialchars($pancard),
                    'email' => htmlspecialchars($email),
                    'state_id' => htmlspecialchars($city_state_id),
                    'city_id' => htmlspecialchars($city_id),
                    'customer_lead_finbox_cust_id' => htmlspecialchars($finbox_customer_id),
...
//            $this->form_validation->set_rules("first_name", "First Name", "required|trim");
            if ($this->form_validation->run() == FALSE) {
                ...
                $mobile = !empty($post['mobile']) ? htmlspecialchars($post['mobile']) : 0;
                $lead_id = htmlspecialchars($post['lead_id']);
    ...
    public function qdeAppSaveCustomer_post() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && $header_validation) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $first_name = htmlspecialchars(strtoupper($post['first_name']));
                $middle_name = htmlspecialchars(strtoupper($post['middle_name']));
                $sur_name = htmlspecialchars(strtoupper($post['sur_name']));
                $email = htmlspecialchars(strtoupper($post['email_personal']));
...
//                $source = strtoupper($post['source']);
                $pancard = htmlspecialchars(strtoupper($post['pancard']));
...
//                $utm_campaign = !empty($post['utm_campaign']) ? strtoupper($post['utm_campaign']) : "";
                $gender = htmlspecialchars(strtoupper($post['gender']));
...
//                $open_screen = "KYCDOCS";
                ...
                $result_data = array('Status' => 1, 'Message' => 'Data Found', 'data' => $lead_customer, 'office_data' => $office_data, 'bankdetails_data' => $bankdetails_data, 'open_screen' => $open_screen);
    ...
    public function qdeAppGetCityName_post() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $city_name = htmlspecialchars(trim($post['cityname']));
                $sourceable = htmlspecialchars(trim($post['sourceable']));
    ...
    public function GetCity_post() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && $header_validation) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $state_id = htmlspecialchars($post['state_id']);
...
//*********** Api for Get Pupose of loan / End Use *************//
    ...
    public function send_notification($registatoin_ids, $notification, $device_type) {
        ...
        // Firebase API Key
        $headers = array('Authorization:key=AAAAuaBHncI:APA91bEiVKo0W6dmiIMHIljqlFVbypyP0mCTTIwxL63XVN3zHcJNwkd90t_xIlZcvQ5QghgY6oOFAFWxX33fg_mhfkMc8u_HWfciEsPnRqZDZVWR1W5X1v03JEapvLsBTF-J5WgcnlUO', 'Content-Type:application/json');
        // Open connection
        $ch = curl_init();
