...
class Android1Controller extends REST_Controller {
    ...
    public function qdeCsPersonalDetails_post()  {
        ...
        // if ($_SERVER['REQUEST_METHOD'] == 'POST' && $header_validation) {
            ...
            $this->form_validation->set_rules("city_id", "City ID", "required|trim");
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $tenure = intval($post['tenure']);
                $gender=intval($post['gender']);
                $address=intval($post['address']);
                $monthly_salary=intval($post['salary']);
            ...
            //     $mobile = !empty($leadId->mobile) ? $leadId->mobile : '';
                ...
                if (!empty($dedupeReturnArray['status']) && $dedupeReturnArray['status'] == 1) {
                    ...
                    $update_personal_details = [
                            ...
                            'user_type' => "NEW",
                            // 'stage' => 'S1',
                            'monthly_salary_amount' => $monthly_salary,
                   ...
                   return json_encode($this->response(['Status' => 1, 'Message' => "Update Personal Detail Successfully" , 'lead_id' => $lead_id,  'mobile' => $mobile], REST_Controller::HTTP_OK));
                }
