        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && $header_validation) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($version == $current_version) {
                    return json_encode($this->response(['Status' => 1, 'Message' => "Success", 'version' => $version, $token], REST_Controller::HTTP_OK));
                } else {
            ...
            return json_encode($this->response(['Status' => 0, 'Message' => 'Request Method Post Failed.',  $token], REST_Controller::HTTP_OK));
        }
    }
...
//                        'monthly_income' => $monthly_income,
                        'salary_mode' => htmlspecialchars($salary_mode),
                    );
    ...
    public function qdeAppSaveResidenceAddresss_post() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && $header_validation) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $residence_since = !empty($post['resi_since_date']) ? date("Y-m", strtotime($post['resi_since_date'])) : "";
...
//                $residence_since .= !empty($post['residence_since_month']) ? "-" . $post['residence_since_month'] : "";
                ...
                $customer_emp = array(
                    ...
                    'emp_residence_since' => htmlspecialchars($office_since_date),
...
//        header('Content-Type: application/json; charset=utf-8');
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {// && $header_validation
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $docs_extension = strtolower(trim($post['ext']));
    ...
    public function qdeAppSaveRegistration_post() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && $header_validation) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $full_name = htmlspecialchars(strtoupper($post['first_name']));
                $mobile = htmlspecialchars($post['mobile']);
                $email = htmlspecialchars(strtoupper($post['email']));
                $city_id = htmlspecialchars($post['city_id']);
                $coordinates = !empty($post['coordinates']) ? htmlspecialchars($post['coordinates']) : '';
                $source_id = !empty($post['source_id']) ? htmlspecialchars($post['source_id']) : 2;
                ...
                $lead_mobile_android_id = !empty($post['lead_mobile_android_id']) ? htmlspecialchars($post['lead_mobile_android_id']) : '';

                $pancard = !empty($post['pancard']) ? htmlspecialchars(strtoupper($post['pancard'])) : "";
                $income_type = !empty($post['income_type']) ? htmlspecialchars($post['income_type']) : "";
                $purposeofloan = !empty($post['purpose_of_loan']) ? htmlspecialchars($post['purpose_of_loan']) : "";
                $loan_amount = !empty($post['loan_amount']) ? htmlspecialchars($post['loan_amount']) : "";
                $monthly_salary = !empty($post['monthly_income']) ? htmlspecialchars($post['monthly_income']) : "";
    ...
    public function qdeAppGetLeadDetails_post() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && $header_validation) {
            ...
            if (!empty($lead_id)) {
                ...
                $num_rows1[0]['loan_amount'] = strval(intval($num_rows[0]['loan_amount']));
                $num_rows = $num_rows1;
...
//                }
                ...
                return json_encode($this->response(['Status' => $status, 'Required_Documents' => $docs_lists, 'Message' => $Message], REST_Controller::HTTP_OK));
            }
        } else {
            return json_encode($this->response(['Status' => 0, 'Message' => 'Request Method Post Failed.'], REST_Controller::HTTP_OK));
        }
