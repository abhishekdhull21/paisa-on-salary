...
//                }
                ...
                require_once (COMPONENT_PATH . 'CommonComponent.php');
                $CommonComponent = new CommonComponent();                
                $CommonComponent->payday_sms_api(1, $lead_id, $data);
//                $this->Tasks->sendOTPForUserRegistrationVerification($data);
    ...
    public function qdeAppOtpVerify_post() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && $header_validation) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $query = $this->db->select('lead_id, mobile, lead_status_id, pancard, user_type')->where(['lead_id' => $lead_id])->from('leads')->get();
    ...
    public function qdeAppLoanStatus_post() {
        ...
        $header_validation = (($headers['Accept'] == "application/json") && ($token['token_chatbot'] == $headers['Auth']));
