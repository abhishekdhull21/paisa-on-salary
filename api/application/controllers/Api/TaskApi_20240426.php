        ...
        // if ($_SERVER['REQUEST_METHOD'] == 'POST' && $header_validation) {
            ...
            if ($this->form_validation->run() == FALSE) {
               ...
               // $mobile = intval($post['mobile']);
                ...
                $city_id = intval($post['city_name']);
                $loan_amount = !empty($post['loan_amount']) ? doubleval($post['loan_amount']) : "";
                $first_name = !empty($post['first_name']) ? strval($post['first_name']) : "";
               ...
               $last_row = $this->db->select('lead_id, mobile')->where('lead_active', $flag)->from('leads')->order_by('lead_id', 'desc')->limit(1)->get();
                ...
                $dedupeRequestArray = array('mobile' => $mobile);
                ...
                if (!empty($dedupeReturnArray['status']) && $dedupeReturnArray['status'] == 1) {
                    ...
                    $last_row = $this->db->select('lead_id')->where('lead_active', $flag)->from('leads')->order_by('lead_id', 'desc')->limit(1)->get();
                    $leadId = $last_row->row();
                    $lead_id = !empty($leadId->lead_id) ? $leadId->lead_id : 0;
                ...
                $insertLeadsCustomer = array(
                    ...
                    'customer_id' => intval($finbox_customer_id),
                    'created_date' => date('Y-m-d H:i:s'),
                );
                ...
                $array = ['Status' => 1, 'Message' => 'Personal Detail Save successfull.', 'lead_id' => $lead_id,  'mobile' => $mobile];
        ...
        // if ($_SERVER['REQUEST_METHOD'] == 'POST' && $header_validation) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($mobile == "9717882592") {//Hardcoded otp testing... Rohit - Don't touch this
                    ...
                    $otp = 4444;
                } else if ($mobile == "9319062592") {//Hardcoded otp testing... Javed
                    $otp = 1906;
                }
     ...
     function verificationAPI_post() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $this->form_validation->set_rules("lead_id", "Lead ID", "required|trim|numeric|is_natural|regex_match[/^[0-9]+$/]");
                ...
                $lead_id = !empty($leadId->lead_id) ? $leadId->lead_id : 0;
                if(!empty($lead_id)){
                    $this->db->where('lead_id', $lead_id)->update('leads', ['pancard' => $panNumber]);
                }
                if(!empty($lead_id)){
                    $this->db->where('Customer_lead_id', $lead_id)->update('lead_customer', ['pancard' => $panNumber]);
                }
                 if(!empty($lead_id)){
                ...
                // try{
                    // $pan_veri_return = $CommonComponent->call_pan_verification_api($lead_id);
                                        print_r($pan_veri_return);

                // }catch(ErrorException $e){
                //     print_r($e);
                // }

                    if (!empty($lead_id)&&!empty($panNumber)) {

                      if($pan_veri_return['status']==1){

                       return json_encode($this->response(['Status' => 1, 'Message' => 'Pancard Verified Successfully', 'lead_id' => $lead_id], REST_Controller::HTTP_OK));
                      }else{
                         return json_encode($this->response(['Status' => 2, 'Message' =>$pan_veri_return['errors'] , 'lead_id' => $lead_id], REST_Controller::HTTP_OK));
                      }
                    } else {

                        return json_encode($this->response(['Status' => 0, 'Message1' => $pan_veri_return['errors'], 'lead_id' => $lead_id], REST_Controller::HTTP_OK));
                    }
...
//         return json_encode($this->response(['Status' => 2, 'Message' => strip_tags(validation_errors())], REST_Controller::HTTP_OK));
//     } else {
//          $salaryFile = $_FILES['salary'];
//         $bankFile = $_FILES['bank'];
//         // Check if files are uploaded successfully
//         if (!isset($_FILES['salary']) || !isset($_FILES['bank'])) {
//             return json_encode($this->response(['Status' => 2, 'Message' => 'Files not uploaded.'], REST_Controller::HTTP_OK));
//         }

//         // Specify upload directory
//         $upload_dir = 'public_html/salaryontime.in/upload/docs/';

//         // Move uploaded files to the specified directory
//         $salary_file_path = $upload_dir . basename($_FILES['salary']['name']);
//         $bank_file_path = $upload_dir . basename($_FILES['bank']['name']);

//         if (move_uploaded_file($_FILES['salary']['tmp_name'], $salary_file_path) &&
//             move_uploaded_file($_FILES['bank']['tmp_name'], $bank_file_path)) {
...
// }
    ...
    // if ($_SERVER['REQUEST_METHOD'] == 'POST' && $header_validation) {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($responseData) {
                $flag=1;
                $last_row = $this->db->select('customer_lead_id')->where('customer_active', $flag)->from('lead_customer')->order_by('customer_lead_id', 'desc')->limit(1)->get();
                $leadId = $last_row->row();
                $lead_id = !empty($leadId->customer_lead_id) ? $leadId->customer_lead_id : 0;

                $update_aadhar_no = array(
                    'aadhar_no' => substr(822492351643, -4) // Assuming you want to store only last four digits
                );

              $this->db->where('customer_lead_id', $lead_id)->update('lead_customer', $update_aadhar_no);

                return json_encode($this->response(['message' => 'Aadhar verification successful.', 'Status' => 1], REST_Controller::HTTP_OK));
                } else {
                    return json_encode($this->response(['Status' => 0, 'Message' => 'Unable to verify Aadhar number.'], REST_Controller::HTTP_OK));
                }
        }
...
//                    exit;
                ...
                if (empty($result)) {
                    ...
                    $subject = "Kasar Credit & Capital - Confirm your email on  Kasar Credit & Capital";
                    ...
                    $html = '<!DOCTYPE html>
                                    ...
                                    <tr>
                                    <td width="975" align="center"><img src="https://lms.sotcrm.com/public/images/brand_logo.png" style="width:150px;"></td>
                                    </tr>
                                    <tr>
                                    <td style="text-align:center;"><table width="618" border="0" style="text-align:center;padding:20px;background: #fff;">
                                    <tr>
                                    <td width="auto" align="center"><img src="https://www.lms.sotcrm.com.com/public/emailimages/verification-email.png" style="width:auto;"></td>
                                    </tr>
                                    <tr>
                                    <td width="612" style="font-size:16px;"><h1>Thank you for joining Kasar Credit & Capital Loan!</h1></td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                    <td align="center">
                                    <a href="https://www.facebook.com/sotcrm-105632195732824" target="_blank">
                                    <img src="https://www.lms.sotcrm.com/public/image/sotcrm-facebook.png" class="socil-t" alt="sotcrm-facebook" style="width:30px;"></a>
                                    <a href="https://twitter.com/sotcrm" target="_blank"><img src="https://www.sotcrm.com/public/image/sotcrm-twitter.png" class="socil-t" alt="sotcrm-twitter" style="width:30px;"></a>
                                    <a href="https://www.linkedin.com/company/sotcrm" target="_blank"><img src="https://www.sotcrm.com/public/image/sotcrm-linkdin.png" class="socil-t" alt="sotcrm-linkdin" style="width:30px;"></a>
                                    <a href="https://www.instagram.com/sotcrm_india" target="_blank"><img src="https://www.sotcrm.com/public/image/sotcrm-instagram.png" class="socil-t" alt="sotcrm-instagram" style="width:30px;"></a>
                                    <a href="https://www.youtube.com/channel/UCUwrJB1IMvDiMctHHRKDLxw" target="_blank"><img src="https://www.sotcrm.com/public/image/sotcrm-youtube.png" class="socil-t" alt="sotcrm-youtube" style="width:30px;"></a>
                                    </td>
                                    </tr>
                                    <tr>
