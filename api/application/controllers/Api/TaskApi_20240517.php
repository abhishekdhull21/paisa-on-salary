...
class TaskApi extends REST_Controller {
   ...
   // public $white_listed_ips = array("208.109.63.229");
    ...
    public function __construct() {
     ...
     public function getDocuments_post($docs_id) {
        ...
        // if ($_SERVER['REQUEST_METHOD'] == 'POST' && $header_validation) {
            ...
            if ($this->form_validation->run() == FALSE) {
               ...
               // $mobile = intval($post['mobile']);
                ...
                $city_name = intval($post['city_name']);
                ...
                // if (!empty($dedupeReturnArray['status']) && $dedupeReturnArray['status'] == 1) {
                   ...
                   $this->db->where('lead_id', $lead_id)->update('leads', $update_personal_details);
                    // $last_row = $this->db->select('customer_lead_id')->where('customer_active', 1)->from('lead_customer')->order_by('customer_lead_id', 'desc')->limit(1)->get();
                    // $leadId = $last_row->row();
                    // $lead_id = !empty($leadId->customer_lead_id) ? $leadId->customer_lead_id : 0;
                   //return json_encode($this->db->where('customer_lead_id', $lead_id)->update('lead_customer', $update_dob));
                ...
                $insertDataLeads = array(
                    ...
                    'user_type' => $user_type,
                    'stage' => 'S1',
                    'state_id' => $getStateName1,
                    'city_id' => $getCityName1,
                    'status' => 'LEAD-NEW',
                    'lead_entry_date' => date('Y-m-d'),
                    'qde_consent' => 'Y',
                    'term_and_condition' => "YES",
                ...
                // prin_r($insertLeadsCustomer); die;
...
//        header('Content-Type: application/json; charset=utf-8');
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {// && $header_validation
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $docs_master = $num_rowsleads[0];
                  ...
                  $image_name = date("YmdHis") . "" . $lead_id . "" . $docs_id;
                  ...
                  $image_upload_dir = "/home/ut8e2mlo1yr5/public_html/salaryontime.in/upload/" . $imgUrl;
    ...
    // if ($_SERVER['REQUEST_METHOD'] == 'POST' && $header_validation) {
        ...
        if ($this->form_validation->run() == FALSE) {
           ...
           if ($responseData) {
               if($responseData->data->otpSentStatus){
                     // Retrieve the Aadhar number from the input data
              ...
              $this->db->where('customer_lead_id', $lead_id)->update('lead_customer', $update_aadhar_no);
                ...
                return json_encode($this->response(['message' => 'Aadhar verification successful.', 'Status' => 1,'result' => $responseData->data->requestId], REST_Controller::HTTP_OK));
                    ...
                    return json_encode($this->response(['Status' => 0, 'Message' => $responseData->error->message], REST_Controller::HTTP_OK));
               }else{
