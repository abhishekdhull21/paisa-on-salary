...
//            $tempDetails = $this->db->select("*")->where(["lead_id" => $lead_id, 'lead_active' => 1, 'lead_deleted' => 0])->from("leads")->get();
    ...
    public function getDisbursementTransDetails($method_id, $lead_id, $log_id, $bank_id) {
        ...
        if (!empty($lead_id)) {
            $tempDetails = $this->db->select("*")->from("api_disburse_logs")->where(["disburse_method_id" => $method_id, "disburse_lead_id" => $lead_id, "disburse_log_id" => $log_id, "disburse_bank_id" => $bank_id, "disburse_active" => 1, "disburse_deleted" => 0])->order_by('disburse_log_id', 'DESC')->get();
            if ($tempDetails->num_rows()) {
                $return_array["status"] = 1;
