...
class Task_Model extends CI_Model {
    ...
    public function index($conditions = null, $limit = null, $start = null, $search_input_array = array(), $where_in = array()) {
        ...
        $select = 'LD.lead_id, DATE_FORMAT(LD.lead_disbursal_recommend_datetime, "%d-%m-%Y %H:%i:%s") as disbursal_recommend, LD.loan_no, LD.customer_id, LD.application_no, LD.lead_reference_no, LD.lead_data_source_id, LD.first_name, C.middle_name, C.sur_name, CONCAT_WS(" ",LD.first_name, C.middle_name, C.sur_name) as cust_full_name,';
...
//        exit;
    ...
    public function getLeadDetails($conditions) {
        ...
        $select .= ' LD.lead_fi_office_status_id,LD.scheduled_date,CAM.loan_recommended as sanctionedAmount, LD.lead_credit_assign_user_id, LD.lead_screener_assign_user_id, LD.lead_disbursal_assign_user_id,';
        $select .= ' user_screener.name as screenedBy,  DATE_FORMAT(LD.lead_screener_assign_datetime, "%d-%m-%Y %H:%i:%s") as screenedOn,';
        $select .= ' user_sanction.name as sanctionAssignTo,  DATE_FORMAT(LD.lead_credit_approve_datetime, "%d-%m-%Y %H:%i:%s"), ';
        $select .= ' L.loan_status_id, DATE_FORMAT(LD.lead_disbursal_approve_datetime, "%d-%m-%Y %H:%i:%s"), L.loan_disbursement_trans_status_id, L.loan_executive_reloan_flag, L.loan_executive_reloan_remark, L.loan_executive_reloan_user_id, ';
...
//        $wherecond = '(status = "SANCTION" OR status= "DISBURSE-PENDING" OR status= "DISBURSED" OR status= "SETTLED" OR status= "WRITEOFF" OR status= "PART-PAYMENT")';
    ...
    public function getCAMDetails($lead_id) {
        ...
        $select .= ' C.father_name,C.gender, C.dob, C.mobile, C.alternate_mobile, LD.obligations, LD.purpose, LD.loan_amount, LD.status, LD.stage, LD.lead_status_id, DATE_FORMAT(LD.lead_disbursal_approve_datetime, "%d-%m-%Y %H:%i:%s"), LD.lead_disbursal_approve_datetime, CAM.loan_recommended,  L.loan_disbursal_letter, ';
    ...
    public function sendDisbursalMail($lead_id, $print_flag = false) {
        ...
        $file_name = "Loan_Desbursal_letter_" . $lead_id . "_" . rand(1000, 9999) . ".pdf";
        ...
        if (file_exists($file_path_with_name)) {
            ...
            $this->updateLeads(['lead_id' => $lead_id], ['loan_disbursal_letter' => $file_name], 'loan');
        } else {
        ...
        $return_array = lw_send_email($email, $subject, $message, $bcc_email, "", "", "", "", $file_name,"disbursal_letter.pdf");
...
//        unlink($file_path_with_name);
    ...
    public function sendSanctionMail($lead_id) {
        ...
        $return_array = lw_send_email($email, $subject, $message, $bcc_email, "", "", "", "", $cam_sanction_letter_file_name, "sanction_letter.pdf");

        if (!empty($alternate_email)) {
            $return_array = lw_send_email($alternate_email, $subject, $message, $bcc_email, "", "", "", "", $cam_sanction_letter_file_name, "sanction_letter.pdf");
        }
...
//            }
    ...
    public function nocSettledPayment($lead_id) {
        ...
        $result = $this->db->select('L.loan_no, CAM.loan_recommended, L.loan_noc_settlement_letter, L.loan_settled_date, CAM.disbursal_date, LD.first_name, LD.email, LD.lead_status_id, CO.date_of_recived,  CO.closure_payment_updated_on, CO.discount')
                ...
                ->join('collection COL', 'COL.lead_id = L.lead_id', 'INNER')
                ->order_by('CO.id', 'desc');
        ...
        if ($res->num_rows() > 0) {
            ...
            $file_name = "Noc_settlement_letter_" . $lead_id . "_" . rand(1000, 9999) . ".pdf";
            ...
            if (file_exists($file_path_with_name)) {
                ...
                //  $return_array['file_name'] = $upload_return['file_name'];
                $file_name = $upload_return['file_name'];
                $this->updateLeads(['lead_id' => $lead_id], ['loan_noc_settlement_letter' => $file_name], 'loan');
            } else {
                $return_array['errors'] = "File does not exist. Please check offline";
            }
            ...
            // $loan_disbursal_letter = file_get_contents(downloadDocument($filepdf, 2));
                ...
                $return_array = lw_send_email($customer_email, $subject, $message, BCC_NOC_EMAIL, "", "", "", "", $file_name, "settlement_letter.pdf");
            ...
            if ($return_array['status'] == 1) {
                ...
                $loan_data = array(
                    ...
                    'loan_noc_settled_letter_user_id' => $user_id,
                    'loan_noc_settled_letter_datetime' =>  date("Y-m-d H:i:s"),
            ...
            //     unlink($file_path_with_name);
            //     //$this->updateLeads(['lead_id' => $lead_id], ['cam_sanction_letter_file_name' => $file_name], 'credit_analysis_memo');
            //     $this->updateLeads(['lead_id' => $lead_id], ['loan_noc_settlement_letter' => $upload_return['file_name']], 'loan');
            // } else {
...
//            traceObject($this->db->error());
    ...
    public function sent_loan_closed_noc_letter($lead_id) {
        ...
        $sql .= " CAM.repayment_date, CAM.repayment_amount, L.loan_closure_date, L.loan_noc_closing_letter";
        ...
        if (!empty($to)) {
           ...
           $subject = 'NOC Closing Letter';
            ...
            $return_array = lw_send_email($to, $subject, $message, BCC_NOC_EMAIL, "", "", "", "", $file_name, "noc_letter.pdf");
                    ...
                    <html xmlns="http://www.w3.org/1999/xhtml">
                        ...
                        <body>
                            <table width="550" border="0" align="center" cellpadding="0" cellspacing="0" style="border:solid 1px #0463a3;">
                                <tr>
                                    <td><a href="' . WEBSITE_URL . '" target="_blank"><img src="' . EMAIL_BRAND_LOGO . '" alt="surya-logo" style="border-radius: 11px;padding: 6px 8px;"></a></td>
                                </tr>
                                <tr>
...
//        die();
    ...
    public function get_mapped_credit_users($credit_head_id, $role_type_id, $user_case_type) {
        ...
        $sql .= " INNER JOIN user_lead_allocation_log ULA on (ULA.ula_user_id = UR.user_role_user_id AND ULA.ula_user_created_on='$today_date' AND ULA.user_active_status=1)";
        $sql .= " WHERE UR.user_role_type_id = $role_type_id AND ULA.user_active_case_type=$user_case_type AND UR.user_role_supervisor_role_id = ";
...
//    }
    ...
    public function generate_Repay_Link_Mail($lead_id)
    {
        $sql = $this->getCAMDetails($lead_id);
        $camDetails = $sql->row();
        ...
        $conditions = ['LD.lead_id' => $lead_id];
        $result = $this->db->select('LD.first_name, LD.email, DATE_FORMAT(CO.date_of_recived, "%d-%m-%Y %H:%i:%s") as date_of_recived, US.name, US.mobile, CO.collection_executive_user_id, LD.lead_status_id')
                // ->where('CO.repayment_type', 17) // SETTLED
                ->where($conditions)
                ->from('leads LD')
                ->join('credit_analysis_memo CAM', 'CAM.lead_id = LD.lead_id', 'INNER')
                ->join('loan L', 'L.lead_id = LD.lead_id', 'INNER')
                ->join('collection CO', 'CO.lead_id = LD.lead_id', 'INNER')
                ->join('users US', 'US.user_id = LD.lead_credit_approve_user_id', 'LEFT')
        ...
        if ($res->num_rows() > 0) {
            ...
            $credit_manager_mobile = $sql->mobile;
            $creditmanager = $sql->name;
            $date_of_recived = $sql->date_of_recived;
            $collection_executive_user_id = $sql->collection_executive_user_id;
            $lead_status_id = $sql->lead_status_id;
            $loan_no = $sql->loan_no;
            $customer_name = $sql->first_name;
            $customer_email = $sql->email;
        ...
        $url_replayment_link =   REPAYMENT_REPAY_LINK.'/'.$this->encrypt->encode($lead_id).'/'.$this->encrypt->encode($user_id);

        require_once (COMPONENT_PATH . 'CommonComponent.php');
        $CommonComponent = new CommonComponent();

        $res = $CommonComponent->call_url_shortener_api($url_replayment_link);
        ...
        if(!empty($lead_id)) {
            ...
            $req['lead_id'] = $lead_id;
            $req['loan_no'] = $camDetails->loan_no;
          //  $req['lead_status_id'] = $lead_cust->lead_status_id;
            $req['name']    = $lead_cust->first_name;
            $req['mobile']  =  $lead_cust->mobile;
            $req['credit_manager']  =  $camDetails->name;
            $req['Repayment_date']  =  $camDetails->lead_credit_assign_datetime;
            $req['email']   =  $lead_cust->email;
            ...
            $message = '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
                        ...
                        <tr>
                        <td>Dear <strong>' . $req['name'] . ', </strong></td>
                        </tr>
                        <tr>
                        <td>&nbsp;
                        </td>
                        ...
                        <tr>
                        <td style = "line-height:25px;">Your Loan No: <strong>' . $req['loan_no'] . '</strong> is due for repayment on <strong>' . $date_of_recived . '.</strong> Please repay your loan within time to save penalty interest.</td>
                        </tr>
                        <tr>
                        <td>&nbsp;
                        </td>
                        ...
                        <tr>
                        <td style = "line-height:25px;">For any queries or assistance, please keep in touch with <strong>' . $req['name'] . ' @ <a style = "text-decoration: none;" href = "tel:' . $req['mobile']  . '">' . $req['name'] . '</a></strong>.</td>
                        </tr>
                        <tr>
                        <td>&nbsp;
                        </td>
                        ...
                        <tr>
                        <td style = "text-align:center;"><a href = "' . $req['repayment_link_generate'] .'" target = "_blank" style = "background: #006;border-radius: 3px;padding: 8px 30px;color: #fff;text-decoration: blink;font-weight: bold;">Repay Loan</a> </td>
                        </tr>
                        <tr>
                        <td>&nbsp;
                        </td>
                        ...
                        <strong>Team '.BRAND_NAME.'</strong><br/>
                        <span>'.COMPANY_NAME.'</span><br/>
                        <span>'.REGISTED_ADDRESS.'</span><br/>
                        </p>
                        </td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                        <td colspan = "3" align = "center" bgcolor = "#0463A3" style = "padding:10px; color:#fff; font-size:14px; font-weight:bold;"><a href = "tel:+91-9999999341" style = "color:#fff; text-decoration:blink;">' . REGISTED_MOBILE . '</a> | <a href = "' . WEBSITE_URL . '" target = "_blank" style = "color:#fff; text-decoration:blink;">www.surya.com</a> | <a href = "mailto:' . INFO_EMAIL . '" style = "color:#fff; text-decoration:blink;">' . INFO_EMAIL . '</a></td>
                        </tr>
            ...
            $subject = 'Repayment Request Link';

            $return_array = lw_send_email($req['email'], $subject, $message, BCC_NOC_EMAIL, "", "", "", "","", "");

            if ($return_array['status'] == 1) {
                $CommonComponent->payday_sms_api($sms_type_id, $lead_id, $req);
                $lead_remark = "Repay Request successfully";
                $data = "true";
            } else {
                $lead_remark = "Repay Request failed";
                $data = "false";
            }
    ...
    public function send_Link_For_Customer_On_Mailer($lead_id,$customer_email,$customer_name) {
        //$customer_email = 'ajit.singh@surya.com';
        $send_url = WEBSITE_URL.'document-upload/'.$this->encrypt->encode($lead_id);
        require_once(COMPONENT_PATH . 'CommonComponent.php');
        $CommonComponent = new CommonComponent();
        $res = $CommonComponent->call_url_shortener_api($send_url);
        ...
        $message = '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
            <html xmlns="http://www.w3.org/1999/xhtml">
                ...
                <body>
                    <table width="778" border="0" align="center" cellpadding="0" cellspacing="0" style="padding:10px; border:solid 1px #ccc; font-family:Arial, Helvetica, sans-serif;">
                        <tr>
                            <td align="center"><img src="' . EMAIL_BRAND_LOGO . '" width="180" height="85" /></td>
                        </tr>
                        <tr>
                            <td><hr style="background:#ddd !important;"></td>
                        </tr>
                        ...
                        <tr>
                            <td align = "center" style = "text-align:center;"><a href = "'.$short_url.'" target = "_blank" style = "background:#0463a3;border-radius: 3px;padding: 8px 30px;color: #fff;text-decoration: blink;font-weight: bold;">Click Here</a></td>
                        </tr>
                        <tr>
                            <td><p style = "margin:0px;color: #000;line-height: 25px;border-radius: 3px;">Please upload documents file.</p></td>
                        </tr>
        ...
        $return_array = lw_send_email($customer_email, 'Upload Document FORM | ' . WEBSITE, $message, "", "", "", CARE_EMAIL);
        return $return_array;
    }

    public function send_Legal_Notice_On_Mailer($lead_id,$customer_email,$customer_name) {
        //$customer_email = 'ajit.singh@surya.com';
        $sql = "SELECT LD.lead_status_id, concat_ws(' ', LC.first_name, LC.middle_name, LC.sur_name) as customer_name,CAM.disbursal_date,LC.gender,LD.email,CAM.tenure,CAM.loan_recommended,LD.loan_no,LC.aa_current_house,LC.aa_current_locality,LC.aa_current_landmark,LC.aa_cr_residence_pincode,LC.aa_current_district,MS.m_state_name State,MC.m_city_name City,LN.loan_total_received_amount,LN.loan_principle_payable_amount";
        $sql .= " FROM leads LD INNER JOIN lead_customer LC ON LC.customer_lead_id=LD.lead_id INNER JOIN credit_analysis_memo CAM ON CAM.lead_id=LD.lead_id INNER JOIN loan LN ON LN.lead_id=LD.lead_id LEFT JOIN master_state MS ON MS.m_state_id=LC.aa_current_state LEFT JOIN master_city MC ON MC.m_city_id=LC.aa_current_city";
        $sql .= " WHERE LD.lead_id=$lead_id";
        $sql = $this->db->query($sql)->row();
        ...
        $loan_principle_payable_amount = $sql->loan_principle_payable_amount;
        $total_due_amount = $loan_principle_payable_amount - $loan_total_received_amount;
        $interest_amount = (($loan_principle_payable_amount * (18*100/365) * $tenure) / 100);
        $total_final_amount = $total_due_amount + $interest_amount;
        $subject = 'Legal Notice FORM | ' . WEBSITE;
        if($gender=='FEMALE'){
        ...
        $this->db->insert('loan_legal_notice_logs',$insertData);
        ...
        <html xmlns="http://www.w3.org/1999/xhtml">
            ...
            <body>
                <table  style="position:relative;width:892px;height:1262px; margin: 0 auto;">
                  <tbody>
                    <tr>
                      <td>
                        <table>
                          <tbody>
                            <tr><img width="892" height="200" src="https://www.surya.com/public/image/adv_header.jpg" alt="background image"/></tr>
                            <tr>
                              <p>DELHI,
                                <br>
                                ' . date('d,M Y') . '
                        ...
                        <tr>
                          <p>TO,
                            <br>
                            ' . $customer_name . '</p>
                            <p> ' . $sql->aa_current_house.', '.$sql->aa_current_locality.'<br>'.$sql->aa_current_landmark. '<br> ' . $sql->State.','.$sql->City.'-'.$sql->aa_cr_residence_pincode. '</p>
                        </tr>
                            <tr>
                              <b style="font-size: 20px;margin-left:400px;">Legal Notice</b>
                            </tr>
                            <tr>
                                <p>
                                  <span> <b> ' . $customer_name . ',</b></span>
                                  <br>Under instructions from and on behalf of my client DEVMUNI LEASING & FINANCE LIMITED With the brand name “Bharat Loan” having its '.REGISTED_ADDRESS.', I address you as under. </p>
                            </tr>
                         <tr>
                            <p>1. That you had approached my client for a short term loan as you were in dire need of money on '.date('d,M Y',strtotime($sql->disbursal_date)). '. </p>
                        </tr>
                         <tr>
                               <p>2.  That Pursuant to the terms and condition of the

                        customer agreement form as agreed by you, you were provided
                        the short-term loan of Rs. '.number_format($loan_principle_payable_amount).' with Loan
                        no-'.$sql->loan_no. '.  </p>
                            </tr>
                        <tr>
                          <p>  3.That it has been more than '.$tenure.' days since you contacted
                        my client regarding the outstanding loan amount and interest
                        that you are bound to payback to my client DEVMUNI LEASING &
                        FINANCE LIMITED  With the brand name “Bharat Loan” .</p>
                        </tr>
                        <tr>
                          <p> 4. That  thus  by  your  act  and  conduct,  it  is  evident  that
                        since  the  time  of  availing  such  loan  you  had  malafide
                        intention.
                        </p>
                        </tr>
                        <tr>
                          <p> 5.  That below mentioned are the details of the loan </p>
                        </tr>
                          </tbody>
                        </table>
                        <table border="2" style="margin: 20px 0; float: left; width: 100%; padding: 9px; border-radius: 10px;background-color: #fefefe;">
                          <tr>
                            <th style="padding: 7px; width: 50%;">Particulars</th>
                            <th style="padding: 7px; width: 50%;">Amount</th>
                          </tr>
                          <tr>
                            <td style="padding: 7px;">Principal loan</td>
                            <td>&nbsp;&nbsp;Rs. '.number_format($sql->loan_recommended).'</td>
                          </tr>
                          <tr>
                            <td style="padding: 7px;">Payment Received</td>
                            <td>&nbsp;&nbsp;Rs. '.number_format($loan_total_received_amount).'</td>
                          </tr>
                          <tr>
                            <td style="padding: 7px;">Interest %</td>
                            <td>&nbsp;&nbsp;18%</td>
                          </tr>
                          <tr>
                            <td style="padding: 7px;">Total Outstanding Balance</td>
                            <td>&nbsp;&nbsp;Rs. '.number_format($total_due_amount).'</td>
                          </tr>

                          <tr>
                            <td style="padding: 7px;">Final Total </td>
                            <td>&nbsp;&nbsp;Rs. '.number_format($total_final_amount).'</td>
                          </tr>
                        </table>
                        <table>
                          <tbody>
                            <tr>
                              <p>6.   That  as  on  date  an  amount  of Rs. '.number_format($total_final_amount).' (Settlement Amount) is  due  and
                                payable by you in the aforesaid connection to our client.</p>
                            </tr>
                            <tr>
                              <p> I, therefore, by means of the present legal notice, call upon
                              you, the notice, to make the payment of the aforesaid amount Rs. '.number_format($total_final_amount).'/- to my client within 7 days of the receipt of this notice
                              by  you,  failing  which  my  client  shall  be  constrained  to
                              initiate legal  proceedings  against  you  under  the  provisions
                              of section 420,468 & 471 of THE INDIAN PENAL CODE,1860 entirely
                              at your Cost, Risk and responsibility.
                              </p>
                            </tr>
                            <tr>
                              <p> A copy of this notice has been retained in my office for further
                              reference, record and action. </p>
                            </tr>
                            <tr>
                              <p style="text-align: right; margin: 3px 0;"><b>Yours Faithfully</b></p>
                            </tr>
                            <tr>
                              <p style="text-align: right; margin: 0 3px;">
                                <img width="100" height="100" src="https://www.surya.com/public/image/adv_sign.png" alt="background image"/>
                              </p>
                            </tr>
                            <tr>
                              <p style="text-align: right; margin: 0 3px;"><b>(HARSH TRIKHA) </b>
                              <br>
         ...
         $pdf_html = '<table style="position:relative;width:892px;height:1262px; margin: 0 auto;">
                    <tr>
                      <td>
                        <img style="width:1400px;height:1262px;" src="https://www.surya.com/public/image/adv_header.jpg" alt="background image"/>
                      </td>
                    </tr>
                    <tr>
                       <td>
                    ...
                    <tr>
                       <td>
                        ' . $customer_name . '<br><br><br>
                       </td>
                    </tr>
                    <tr>
                       <td>
                        ' . $sql->aa_current_house.', '.$sql->aa_current_locality.'<br>'.$sql->aa_current_landmark. '<br> ' . $sql->State.','.$sql->City.'-'.$sql->aa_cr_residence_pincode. '
                       </td>
                    </tr>
                    <tr>
                        <td style="text-align:center;"><b style="font-size: 20px;">Legal Notice</b></td><br><br><br>
                    </tr>
                    <tr>
                       <td>
                          <span> <b> ' . $customer_name . ',</b></span>
                          <br>Under instructions from and on behalf of my client DEVMUNI LEASING & FINANCE LIMITED With the brand name “Bharat Loan” having its '.REGISTED_ADDRESS.', I address you as under. <br><br><br><br>
                       </td>
                    </tr>
                    <tr>
                        <td>1. That you had approached my client for a short term loan as you were in dire need of money on '.date('d,M Y',strtotime($sql->disbursal_date)).'. <br><br><br></td>
                    </tr>
                    <tr>
                        <td>2. That Pursuant to the terms and condition of the customer agreement form as agreed by you, you were provided the short-term loan of Rs. '.number_format($loan_principle_payable_amount).' with Loan
                        no-'.$sql->loan_no.'.<br><br><br></td>
                    </tr>
                    <tr>
                        <td>3.That it has been more than '.$tenure.' days since you contacted my client regarding the outstanding loan amount and interest that you are bound to payback to my client DEVMUNI LEASING & FINANCE LIMITED With the brand name “Bharat Loan”.<br><br><br></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td>5. That below mentioned are the details of the loan.<br><br><br></td>
                    </tr>
                    <tr>
                        <td>
                        <table border="4" style="width: 100%; padding: 9px; border-radius: 10px;background-color: #fefefe;font-size: 15px;font-weight:bold;border: 2px solid #d0e4f5">
                          <tr>
                            <th style="border-radius: 10px;background-color: #fefefe;font-size: 15px;font-weight:bold;border: 2px solid #d0e4f5">Particulars</th>
                            <th style="border-radius: 10px;background-color: #fefefe;font-size: 15px;font-weight:bold;border: 2px solid #d0e4f5">Amount</th>
                          </tr>
                          <tr>
                            <td style="border-radius: 10px;background-color: #fefefe;font-size: 15px;font-weight:bold;border: 2px solid #d0e4f5">Principal loan</td>
                          ...
                          <tr>
                            <td style="border-radius: 10px;background-color: #fefefe;font-size: 15px;font-weight:bold;border: 2px solid #d0e4f5">Interest %</td>
        ...
        $return_array = lw_send_email($customer_email,$subject,$message,"","","","","",$file_name,"legal_notice.pdf");
        return $return_array;
    }

    public function sent_legal_notice_for_customer_sms($lead_id) {
        $send_url = WEBSITE_URL.'document-upload/'.$this->encrypt->encode($lead_id);
        require_once (COMPONENT_PATH . 'CommonComponent.php');
        $CommonComponent = new CommonComponent();
        $sql = 'SELECT LD.lead_id, LC.mobile, LC.first_name as name FROM leads LD INNER JOIN lead_customer LC ON (LC.customer_lead_id=LD.lead_id) WHERE LD.lead_id=' . $lead_id;
        ...
        if ($result->num_rows() > 0) {
            ...
            $response = $CommonComponent->payday_sms_api(1,$lead_id,$sms_request);
        }
    }
