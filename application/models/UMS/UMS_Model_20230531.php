...
class UMS_Model extends CI_Model {
    ...
    public function getCHI($user_id = null) {
        $CHI_List = array();
        $conditions['user_role_type_id'] = 4;
        $conditions['is_Active  '] = 1;
        $conditions['user_status_id  '] = 1;
        $conditions['user_role_active '] = 1;
        ...
        $this->db->select('user_role_user_id, user_role_type_id, user_role_active, user_role_id, user_id, name');
        $this->db->from('user_roles');
        $this->db->join('users', 'user_id = user_role_user_id');
        ...
        foreach ($tempDetails->result_array() as $temp_data) {
            $CHI_List[$temp_data['user_role_id']] = $temp_data['name'];
        }
       // print_r($CHI_List); die;
    ...
    public function getMapped_FCE_with_SCM($user_role_id) {
        $query = $this->db->select('user_role_user_id')->where('user_role_id', $user_role_id)->from('user_roles')->get();
        if ($query->num_rows() > 0) {
            $result = $query->row();
    ...
    public function getSCMSelectedvalue($user_role_id) {
        $query = $this->db->select('user_role_supervisor_role_id')->where('user_role_id', $user_role_id)->from('user_roles')->get();
        if ($query->num_rows() > 0) {
            $result = $query->row();
