...
class CronEmailer_Model extends CronCommon_Model {
    ...
    public function getTodayBirthdayCustomer() {
        ...
        $tempDetails = $this->db->query("SELECT LOWER(LD.email) as user_email_id FROM leads LD INNER JOIN lead_customer LC ON(LC.customer_lead_id = LD.lead_id) WHERE DATE_FORMAT(DATE(LC.dob),'%m-%d')='$dob_val' AND LC.dob IS NOT NULL AND LC.dob!='' AND LD.email!='' AND LD.email IS NOT NULL GROUP BY LD.email");
