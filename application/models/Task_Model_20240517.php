...
class Task_Model extends CI_Model {
    ...
    public function index($conditions = null, $limit = null, $start = null, $search_input_array = array(), $where_in = array()) {
        ...
        $twentySecondsAgo = date('Y-m-d H:i:s', strtotime('-1200 seconds'));
        ...
        $select .= ' LD.lead_fi_office_status_id,LD.scheduled_date,CAM.loan_recommended as sanctionedAmount, DATE_FORMAT(CAM.disbursal_date,"%d-%m-%Y") AS disbursal_date, LD.lead_credit_assign_user_id, LD.lead_screener_assign_user_id, LD.lead_disbursal_assign_user_id,';
        ...
        $select .= ' CAM.repayment_date,  CAM.repayment_amount, CAM.cam_sanction_letter_esgin_on,';
    ...
    public function getLeadDetails($conditions) {
        ...
        $select .= ' LD.email, C.alternate_email, C.gender, LD.mobile, C.alternate_mobile, LD.obligations, LD.promocode, LD.purpose, LD.lead_stp_flag, DATE_FORMAT(LD.lead_final_disbursed_date,"%d-%m-%Y") AS lead_final_disbursed_date, ';
        $select .= ' LD.user_type, LD.pancard, LD.loan_amount, LD.tenure, LD.cibil, CE.income_type, CE.salary_mode, CE.monthly_income, ';
...
//                $result = $this->db->order_by('LD.lead_id', 'DESC')->limit(100)->get();
                ...
                $dedupeSql .= ' REJ.reason as reject_reason, REJU.name as rejected_by_name,CAM.disbursal_date, CAM.lead_final_disbursed_date';
                $dedupeSql .= ' FROM leads LD';
                $dedupeSql .= ' INNER JOIN lead_customer C ON(C.customer_lead_id = LD.lead_id)';
                $dedupeSql .= ' INNER JOIN master_state ST ON(ST.m_state_id = LD.state_id)';
                $dedupeSql .= ' INNER JOIN master_city CT ON(CT.m_city_id = LD.city_id)';
...
//                echo $dedupeSql;
                if ($query_run_flag) {
                    $result = $this->db->query($dedupeSql);
                }
            }
        }
        return $result;
...
//            traceObject($this->db->error());
    ...
    public function sent_loan_closed_noc_letter($lead_id) {
        ...
        if (!empty($to)) {
            ...
            $file_path_with_name = 'https://salaryontime.in/upload/' . $file_name;
            require_once __DIR__ . '/../../vendor/autoload.php';
