...
class Task_Model extends CI_Model {
   ...
   public function index($conditions = null, $limit = null, $start = null, $search_input_array = array(), $where_in = array()) {
        ...
        if ($this->uri->segment(1) == "preclosure" || $this->uri->segment(1) == "closure") {
            ...
            $select .= ',CO.received_amount,CO.date_of_recived, collection_executive.name as collection_executive, CO.collection_executive_payment_created_on as payment_uploaded_on ';
...
/*         if (in_array($this->uri->segment(1), ["collection"])) {
    ...
    public function getLeadDetails($conditions) {
        ...
        $select .= ' LD.email, C.alternate_email, C.gender, LD.mobile, C.alternate_mobile, LD.obligations, LD.promocode, LD.purpose, LD.lead_stp_flag, DATE_FORMAT(LD.lead_final_disbursed_date,"%d-%m-%Y") AS lead_final_disbursed_date, L.legal_notice_letter, ';
        ...
        $select .= ' LD.lead_rejected_assign_user_id, LD.lead_rejected_reason_id,LD.lead_rejected_assign_counter,C.customer_bre_run_flag, CAM.city_category, ';
        $select .= ' MRT.m_marital_status_name as marital_status, MOC.m_occupation_name as occupation, MQ.m_qualification_name as qualification';
        ...
        if ($sql->num_rows() > 0) {
            foreach ($sql->result() as $row) {
                ...
                if (!in_array($row->payment_mode_id, array(2)) || !empty($row->docs)) {
                    $documentViewBtn = '<a class="btn btn-control btn-danger" target="_blank" href="' . base_url('upload/' . $row->docs ) . '" title="' . $row->id . '"><i class="fa fa-eye"></i></a>';
                }
...
//        $wherecond = '(status = "SANCTION" OR status= "DISBURSE-PENDING" OR status= "DISBURSED" OR status= "SETTLED" OR status= "WRITEOFF" OR status= "PART-PAYMENT")';
    ...
    public function getCAMDetails($lead_id) {
        ...
        $select .= ' CAM.eligible_foir_percentage, CAM.admin_fee as processing_fee_percent, CAM.user_id, CAM.cam_sanction_letter_esgin_on, CAM.cam_sanction_letter_file_name, ';
    ...
    public function sendDisbursalMail($lead_id, $print_flag = false) {
        ...
        $message = '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
                        ...
                        $upload_return = uploadDocument($file_path_with_name, $lead_id, 2, 'pdf');
                                ...
                                $return_array = common_send_email($email, $subject, $message, "", "", "", "", $file_path_with_name, $file_name, 'disbursal_letter.pdf');
...
//            $processing_fee_percent = $camDetails->processing_fee_percent;
    ...
    public function nocSettledPayment($lead_id) {
        ...
        if ($res->num_rows() > 0) {
            ...
            $return_array = common_send_email($customer_email, $subject, $message, BCC_NOC_EMAIL, "", "", "", "", $file_name, "settlement_letter.pdf");
...
//            traceObject($this->db->error());
    ...
    public function sent_loan_closed_noc_letter($lead_id) {
        ...
        if (!empty($to)) {
            ...
            $message = '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
                            <html xmlns="http://www.w3.org/1999/xhtml">
                                ...
                                <body>
                                    <table width="778" border="0" align="center" cellpadding="0" cellspacing="0" style="padding:10px; border:solid 1px #ccc; font-family:Arial, Helvetica, sans-serif;">
                                        <tr>
                                            <td align="left"><img src="' . EMAIL_BRAND_LOGO . '" width="320" height="85" /></td>
                                        </tr>
                                        <tr>
                                        ...
                                        <tr>
                                          <td><span style="font-size:17px;line-height: 25px;padding-bottom: 6px; text-align:justify; margin:10px 0px;">
                                            <a href="https://apps.apple.com/app/salaryontime-sot/id6503283983" target="_blank"><img style="width: 64%;margin-left: 55%;" src="https://salaryontime.com/public/images/appleLogo.jpeg"></a></span>
                	                        <span style="font-size:17px;line-height: 25px;padding-bottom: 6px; text-align:justify; margin:10px 0px;"><a href="https://play.google.com/store/apps/details?id=com.salaryontime.salaryontime" target="_blank"><img style="width: 64%;margin-left: 19%;" src="https://salaryontime.com/public/images/playLogo.jpeg"></a></span>
                	                        </td>
                                        </tr>
            ...
            $file_path_with_name = '/home/ut8e2mlo1yr5/public_html/salaryontime.in/upload/' . $file_name;
            require_once __DIR__ . '/../../vendor/autoload.php';
...
try {
    $mpdf = new \Mpdf\Mpdf();
    $mpdf->WriteHTML($message);
    //$mpdf->Output(__DIR__ . '../../upload/' . $file_name, 'F');
    // Optional: output the file to the client browser if needed
    $mpdf->Output($file_path_with_name, 'F');
    ...
    echo 'PDF generation error: ' . $e->getMessage();
            ...
            $return_array = common_send_email($to, $subject, $message, BCC_NOC_EMAIL, "", "", "", "", $file_name, "noc_letter.pdf");
            // print_r($return_array); die;
...
//    }
    ...
    public function generate_Repay_Link_Mail($lead_id) {
        ...
        if (!empty($lead_id)) {
            ...
            $return_array = common_send_email($req['email'], $subject, $message, BCC_NOC_EMAIL, "", "", "", "", "", "");
            ...
            <html xmlns="http://www.w3.org/1999/xhtml">
                ...
                <body>
                    <table width="778" border="0" align="center" cellpadding="0" cellspacing="0" style="border:solid 1px #ccc; font-family:Arial, Helvetica, sans-serif;">
                        ...
                        <tr>
                            <td style="padding:0 10px"><p style = "margin:0px;margin-top:10px;color: #000;line-height: 25px;color:#0463a3">Best Wishes<br>surya</p>
                        </tr>
                        <tr>
                                            <td colspan = "4" align = "center" valign = "middle" style = "border-top:solid 1px #ddd; padding-top:5px;">
...
//        }
                                                                    ...
                                                                    <tr>
                                                                        <td colspan="3" style="border-radius: 11px; color: #000; font-size: 13px;line-height: 18px; text-align: left;padding: 26px;">
                                                                            ...
                                                                            <span width="300" cellpadding="0">
                                                                                ...
                                                                                <br/><br/>
                                                                                We thank you for showing interest in surya Instant personal loan.
    ...
    public function getRepaymentReminderSend($message_type=NULL) {
		...
		$this->db->select('
			...
			CAM.repayment_date,
			DATEDIFF(CAM.repayment_date, CURDATE()) AS days_until_repayment'
		)
		->from('leads LD')
	...
	public function getUserList() {
        ...
        $tempDetails = $this->db->select('*')->from('users')->where_in('role_id', [2])->get();
