...
//                exit;
    ...
    public function RejectionAnalysisModel($fromDate, $toDate, $c4c_ref) {
        if (!empty($fromDate) && !empty($toDate)) {
            ...
            $q = "SELECT LD.lead_id, MS.data_source_name, LD.lead_status_id, master_status.status_name, LD.lead_rejected_reason_id, RJ.reason, ER.lerr_emp_type_flag, ER.lerr_cust_income_flag, ER.lerr_dob_flag, ER.lerr_city_flag, ER.lerr_state_flag, ER.lerr_cust_duplicate_flag, ER.lerr_cust_reject_flag, ER.lerr_cust_blacklisted_flag, ER.lerr_blacklisted_pincode_flag, ER.lerr_salary_mode_flag ";
    ...
    public function LeadAssignmentSummaryModel() {
        ...
        $q = "SELECT U.user_id, U.name, U.email, U.mobile, SUM(IF(LD.lead_id > 0,1,0)) as total_leads, (SELECT IF(LAA.ula_user_status=1,1,0) FROM user_lead_allocation_log LAA WHERE LAA.ula_user_id=U.user_id AND DATE(LAA.ula_created_on)='$today' AND LAA.ula_active=1 ORDER BY LAA.ula_id DESC LIMIT 1) as user_active_flag, (SELECT IF(LFR.ula_user_case_type>0,LFR.ula_user_case_type,0) FROM user_lead_allocation_log LFR WHERE LFR.ula_user_id=U.user_id AND DATE(LFR.ula_created_on)='$today' AND LFR.ula_user_status=1 AND LFR.ula_active=1 ORDER BY LFR.ula_id DESC LIMIT 1) as user_active_case_type ";

        $q .= "FROM users U INNER JOIN user_roles UR ON(U.user_id=UR.user_role_user_id AND UR.user_role_type_id=2) LEFT JOIN leads LD ON(LD.lead_screener_assign_user_id>0 AND LD.lead_screener_assign_user_id=U.user_id AND LD.lead_status_id IN(2,3)) ";
        $q .= "WHERE U.user_id=UR.user_role_user_id AND UR.user_role_type_id=2 AND U.user_is_loanwalle=0 AND U.user_active=1 AND U.user_status_id=1 AND UR.user_role_active=1 GROUP BY U.name ASC ";
    ...
    public function Source_utm_source_status_Model($fromDate, $toDate) {
        if (!empty($fromDate) && !empty($toDate)) {
            ...
            $q .= "WHERE LD.lead_status_id=MS.status_id AND LD.lead_active=1 AND LD.lead_entry_date >= '$fromDate' AND LD.lead_entry_date <= '$toDate' GROUP BY source, utm_source, utm_campaign, status ORDER BY lead_status_id, utm_source, source ASC; ";
    ...
    public function LeadRejectionAnalysisCampaignModel($fromDate, $toDate, $c4c_ref) {
        ...
        if (!empty($fromDate) && !empty($toDate)) {
            ...
            $q = "SELECT LD.lead_id, LD.utm_campaign, MS.data_source_name, LD.lead_status_id, master_status.status_name, LD.lead_rejected_reason_id, RJ.reason, ER.lerr_emp_type_flag, ER.lerr_cust_income_flag, ER.lerr_dob_flag, ER.lerr_city_flag, ER.lerr_state_flag, ER.lerr_cust_duplicate_flag, ER.lerr_cust_reject_flag, ER.lerr_cust_blacklisted_flag, ER.lerr_blacklisted_pincode_flag, ER.lerr_salary_mode_flag ";
            $q .= "FROM leads LD INNER JOIN master_data_source MS ON(LD.lead_data_source_id=MS.data_source_id) INNER JOIN master_status ON(LD.lead_status_id=master_status.status_id) ";
            $q .= "LEFT JOIN lead_eligibility_rules_result ER ON(LD.lead_id=ER.lerr_lead_id) LEFT JOIN tbl_rejection_master RJ ON(LD.lead_rejected_reason_id=RJ.id) ";
    ...
    public function SourceWiseLoanDisbursalReportModel($fromDate, $toDate) {
        ...
        $data .= '<tr>';
        $data .= '<th width="20%" rowspan="11" style="background:#dce6f0;color:#222 !important;" align="center" class="no-of-case">surya</th>';
        ...
        $data .= '<tr>';
        $data .= '<th width="20%" rowspan="5" style="background:#dce6f0;color:#222 !important" align="center" class="no-of-case">surya</th>';
