...
class Verification_Model extends CI_Model {
    ...
    public function getVerificationData($lead_id) {
        $data = array();
        if (!empty($lead_id)) {
            $sql = "SELECT leads.lead_is_mobile_verified, lead_customer.email, lead_customer.email_verified_on, lead_customer.alternate_email, lead_customer.alternate_email_verified_on, lead_customer.email_verified_status, lead_customer.alternate_email_verified_status, scm_residence_user.name as scm_fi_res_name, scm_office_user.name as scm_fi_office_user, rm_residence_user.name as rm_fi_res_name, rm_office_user.name as rm_fi_office_user";
            $sql .= "  ,tbl_verification.residence_initiated_on, tbl_verification.office_initiated_on, tbl_verification.init_residence_cpv, tbl_verification.init_office_cpv  ";
