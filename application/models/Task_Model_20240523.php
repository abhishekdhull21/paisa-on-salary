...
class Task_Model extends CI_Model {
    ...
    public function index($conditions = null, $limit = null, $start = null, $search_input_array = array(), $where_in = array()) {
        ...
        if (in_array($this->uri->segment(1), ["collection"])) {
         ...
         $this->db->where('repayment_date > NOW()', NULL, FALSE); // Just an example, replace with actual normal condition
        }
    ...
    public function enquires() {
		...
		WHERE
			...
			AND (LD.first_name IS NULL OR LD.first_name = "")
			AND LD.mobile NOT IN (
				SELECT DISTINCT mobile
				FROM leads
				WHERE lead_is_mobile_verified = 1
			)
		ORDER BY
			LD.lead_id DESC';
        //echo $sql;
