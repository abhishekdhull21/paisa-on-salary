    ...
    function get_Banking_Analysis_Response_Data($lead_id) {
        ...
        if ($nobel_response_data->num_rows() > 0) {
            ...
            if ($response_data_array['status'] == 'Submitted') {
                ...
                if (strpos(strtolower($response_data_array['message']), 'fraud') !== false) {
                    $return_data .= '<h4 class="footer-support" style="color: red; font-size: 12px;padding-left: 10px;font-weight: bold;">' . $response_data_array['message'] . '</h4>';
                }
                $return_data .= '<h4 class="footer-support" style="color: red; font-size: 12px;padding-left: 10px;font-weight: bold;">Fraud Indicators&nbsp; | Fraud Score : ' . $fraudScore . '&nbsp;&nbsp;<i class="fa fa-angle-double-down" style="user-select: auto;"></i></h4>';
                ...
                $ii = 0;
                foreach ($fraudIndicators as $fraudIdicateValues) {
                    $return_data .= '<div class="table-responsive"><table class="table table-bordered table-striped" style="margin: 0;">';
                    $return_data .= '<tr>';
                    $return_data .= '<td>' . ($ii + 1) . '</td>';
                    foreach ($fraudIdicateValues as $key => $value) {
                        if ($key != 'transactions') {
                            $return_data .= '<td>' . $value . '</td>';
                        } else if (!empty($fraudIndicators[$ii][$key]) && $key == 'transactions') {
                            $return_data .= '<tr><div class="table-responsive"><table class="table table-bordered table-striped">';

                            $return_data .= '<tr>';
                            foreach ($fraudIndicators[$ii][$key][0] as $key => $values) {
                                $return_data .= '<th>' . ucwords($key) . '</th>';
                            }
                            $return_data .= '</tr>';

                            foreach ($fraudIndicators[$ii]['transactions'] as $key_transactions) {
                                $return_data .= '<tr>';
