...
class RejectionController extends CI_Controller {
    ...
    public function resonForRejectLoan() {
                    ...
                    if ($leadsDetails->num_rows() > 0) {
                        ...
                        if ($rejectDetails->num_rows() > 0) {
                            ...
                            $lead_data = array(
                                ...
                                'lead_rejected_datetime' => timestamp,
                            ...
                            if ($sms_sent_flag) {
                                ...
                                $req['lead_id']=$lead_id;
                                $req['mobile']=$leads->mobile;
                                //$req['mobile']=8750256406;
                                $res = $CommonComponent->payday_sms_api($sms_type_id, $lead_id, $req);
                                //print_r($res);
                                //$this->SMS->smsForRejectLeads($lead_id, $leads->first_name, $leads->mobile);
                            }
     ...
     /************************ Lead Reject Again #Create by Rohit Jain **************************/

   public function resonForRejectAgain() {
    if (empty($_SESSION['isUserSession']['user_id'])) {
        $json['errSession'] = "Session Expired.";
        echo json_encode($json);
    }
        ...
        $this->db->select('*');
        $lead_info =  $this->db->get('leads')->row_array();  
     
        $lead_rejected_assign_reason_id  = $lead_info['lead_rejected_assign_user_id'];
        $lead_rejected_reason_id = $lead_info['lead_rejected_reason_id'];
        $lead_status_id  = $lead_info['lead_status_id'];
        $status  = $lead_info['status'];
        $stage  = $lead_info['stage'];
        $lead_rejected_assign_datetime    =  $lead_info['lead_rejected_assign_datetime'];   
      
        $conditions = ['lrr_lead_id' => $lead_id];
        $this->db->where($conditions); 
        $query = $this->db->get('lead_rejection_reasons');
        ...
        if ($query->num_rows() >= 5  ){
            ...
            echo json_encode($data);
                ...
                $user_id    = $_SESSION['isUserSession']['user_id'];
                if( $user_id ){
                $this->db->where('lrr_lead_id', $lead_id);
                $this->db->select('*');
                $total =  $this->db->get('lead_rejection_reasons')->num_rows();
                }

                if(!empty($lead_id)){               
                 if(in_array($lead_rejected_reason_id, array(7,31))){
                    $data = [
                        ...
                        'lead_rejected_assign_counter ' =>  $total+1
                        ];
                   
                    }else{
                        $data = [
                            'lead_rejected_assign_user_id'   => 0,
                            'lead_id'   => $lead_id,
                            'stage'     => 'S9',
                            'status'    => 'REJECT',
                            'lead_rejected_reason_id' => $lead_rejected_reason_id,     
                            'lead_rejected_assign_datetime'  => 0,
                            'lead_rejected_assign_counter ' => 0
                        ]; 
                ...
                'lrr_lead_id' => $lead_id,
                'lrr_user_id' => $_SESSION['isUserSession']['user_id'],
                'lrr_rejected_reason_id' => $user_id,
                'lrr_rejected_remarks' => $status,
...
/************************ End Function  **************************/
    ...
    public function getRejectionReasonAgain() {
        if (product_id == 1) {
            $product_id = 1;
            if ($_SESSION['isUserSession']['labels'] == 'CR1') {
                $whereOnRole = 'user_access = "1" or user_access = "2"';
