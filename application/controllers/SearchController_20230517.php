...
class SearchController extends CI_Controller {
    ...
    public function filter() {
        if (empty($_SESSION['isUserSession']['user_id'])) {
            ...
            $lead_reference_no = trim($this->input->post('lead_reference_no'));
            $loan_no = trim($this->input->post('loan_no'));
            ...
            $querySearch = "SELECT DISTINCT LD.lead_id, LD.customer_id, LL.loan_no, LL.loan_status_id, C.first_name, C.middle_name, C.sur_name, C.email, C.mobile, C.pancard, CAM.loan_recommended as loan_amount_approved, LD.status as credit_status, LD.status, CAM.created_at as credit_date, LD.created_on as lead_date, CT.m_city_name as city ";
            ...
            $datatable = '<table class="table dt-table table-striped table-bordered table-responsive table-hover" style="border: 1px solid #dde2eb">
                            <thead>
                                <tr>
                                    ...
                                    <th class="whitespace"><b>Initiated&nbsp;Date</b></th>
                                    <th class="whitespace"><b>Credit&nbsp;Date</b></th>
                                </tr>
                            </thead>
                        <tbody>';
            $i = 1;
            if ($query->num_rows() > 0) {
                ...
                foreach ($query->result() as $row) {
                    ...
                    $credit_date = (($row->credit_date) ? date('d-m-Y H:i:s', strtotime($row->credit_date)) : '-');
