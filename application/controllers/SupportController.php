...
class SupportController extends CI_Controller {
    ...
    public function eKYCReset() {
        ...
        if ($agent == 'CA' || in_array($user_id, $this->user_access)) {
	...
	/*
        if (!empty($_POST['city_id'])) {
            $conditions["master_pincode.m_pincode_city_id"] = intval($_POST['city_id']);
        }
	*/
        if(!empty($_POST['filter_input'])) {
            $conditions["(master_pincode.m_pincode_value='".$_POST['filter_input']."')"] = null;
        }

        $config = array();
        $config["base_url"] = $url;
        $config["total_rows"] = $this->support->pincodeListCount($conditions);
        $config["per_page"] = 10;
        ...
        $data['pageURL'] = $url;
        $data['pincodeDetails'] = $this->support->pincodeList($config["per_page"],$page,$conditions);
    ...
    public function addPincode() {
        ...
        $view_data = array();
        $m_pincode_data = array();
        $success_msg = "";
        $errors_msg = "";
        $update_flag = false;
        $m_pincode_id = 0;
        if($this->input->server('REQUEST_METHOD') == 'POST') {
        echo 'Testing PINCODE';
            $m_pincode_data["m_pincode_id"]        = $this->input->post('m_pincode_id');
            $m_pincode_data["m_pincode_value"]   = $this->input->post('m_pincode_value');
            $this->form_validation->set_rules('master_pincode ','Pincode','required');
            if($this->form_validation->run() == FALSE) {
                $errors_msg = validation_errors();
            } else {
                    echo 'Testing else';
                if($this->support->checkSupportPincode($m_pincode_data["m_pincode_value"])) {
                    $errors_msg = "Pincode is already exist. Please enter new pincode";
                } else {
                    echo 'Testing error';
                    $insert_m_pincode_array = array();
                    $insert_m_pincode_array["m_pincode_id"]       = $m_pincode_data["m_pincode_id"];
                    $insert_m_pincode_array["m_pincode_value"]    = $m_pincode_data["m_pincode_value"];
                    $insert_m_pincode_array["m_pincode_city_id"]  = $m_pincode_data["m_pincode_city_id"];

                    $mpincode_id = $this->db->insert('master_pincode',$insert_m_pincode_array);
                    print_r($mpincode_id); die;
                    if(!empty($m_pincode_id)) {
                        $success_msg = "Pincode has been added successfully.";
                        $this->session->set_flashdata('success_msg',$success_msg);
                        $enc_mbp_id = $this->encrypt->encode($m_pincode_id);
                        return redirect(base_url('support/pincodeList'),'refresh');
                    } else {
        ...
        $view_data["m_pincode_data"] = $m_pincode_data;
        $this->load->view('Support/addPincode',$view_data);
    }
}
