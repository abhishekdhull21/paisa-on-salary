        ...
        if (true) {
            ...
            switch ($labelname) {
                ...
                case 'CR2': // Credit (Credit Manager)
                    $where .= " AND is_active='1' AND (user_labels='CO1' OR user_labels='CR2' OR user_labels='CR3' OR user_labels='REJ')";
                    break;
                case 'CR3': // Credit (Credit Head)
                    $where .= " AND is_active='1' AND (user_labels='CC' OR user_labels='CR1' OR user_labels='CR2' OR user_labels='CR3' OR user_labels='DS1' OR user_labels='DS2' OR user_labels='SA' OR user_labels='REJ')";
                    break;
                case 'DS1': // Disbursal (Disbursal Team)
                    $where .= " AND is_active='1' AND (user_labels='DS1' OR user_labels='DS2' OR user_labels='SA')";
                    break;
                case 'DS2': // Disbursal (Disbursal Team)
                    $where .= " AND is_active='1' AND (user_labels='CC' OR user_labels='DS1' OR user_labels='DS2' OR user_labels='SA')";
                    break;
                ...
                case 'CO2': // collection (collection SCM)
                    $where .= " AND is_active='1' AND (user_labels='CFE1' OR user_labels='CO1' OR user_labels='CO2' OR user_labels='CO3' OR user_labels='SA')";
                    break;
                ...
                case 'AC1': // collection (Closing Team)
                    $where .= " AND is_active='1' AND (user_labels='AC1' OR user_labels='CO3' OR user_labels='AC2' OR user_labels='CA' OR user_labels='SA')";
                    break;
                case 'AC2': // collection (Closing Team)
                    $where .= " AND is_active='1' AND (user_labels='CC' OR user_labels='AC1' OR user_labels='CO3' OR user_labels='AC2' OR user_labels='DS2' OR user_labels='CA' OR user_labels='SA')";
                    break;
                case 'MR': // Marekting
                ...
                case 'CC': // Customer Care
                    $where .= " AND is_active='1' AND (user_labels='CC' OR user_labels='CO1' OR user_labels='CR1' OR user_labels='CR2' OR user_labels='CR3' OR user_labels='AC2' OR user_labels='DS2' OR user_labels='REJ')";
                    break;
                case 'AU': // Audit
                    $where .= " AND is_active='1' AND user_labels in('AU','OL','CR1','CR2','CR3','DS1','DS2','AC1','AC2','CO1','CO2','CO3','CC','REJ')";
                    break;
                default:
                    $where .= " AND is_active='1' AND  user_labels='OL'";
                    break;
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (!empty($isValidUser['status'])) {
                    ...
                    if (!empty($isValidUser['user_data']['email'])) {
                        lw_send_email($isValidUser['user_data']['email'], $email_subject, $email_message, CTO_EMAIL);
                    }
                    ...
                    if ($userDetails['status'] == 1) {
                        ...
                        if ($userDetails['user_data']['user_logins_failed_count'] >= 3) {
                            ...
                            if (!empty($userDetails['user_data']['email'])) {
                                lw_send_email($userDetails['user_data']['email'], $email_subject, $email_message, CTO_EMAIL);
                            }
                        } else {
    ...
    public function verifyUser() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($result->num_rows() > 0) {
                ...
                $message = "<table width='650' border='1' cellspacing='0' cellpadding='0' style='border:1px solid #000'>
                                    <tr bgcolor='#ededed'>
                ...
                lw_send_email($email, $subject, $message, CTO_EMAIL);
        ...
        if (!empty($_SESSION['isUserSession']['user_id'])) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (!in_array(agent, ["CR1"])) {
                    $error = array('errormessage' => 'Only Screener can save the allocation.', 'status' => 0);
                    echo json_encode($error);
                } else {
