...
//echo $sql->num_rows(); die;
    ...
    public function send_settlement_letter($lead_id) {
        ...
        if (!empty($lead_id)) {
            $data = $this->Tasks->nocSettledPayment($this->encrypt->decode($lead_id));
            if ($data == "false") {
    ...
    public function downloadNocSettlementLetter($enc_lead_id) {

        if (!empty($enc_lead_id)) {
            $lead_id = intval($this->encrypt->decode($enc_lead_id));
            $return_array = array("status" => 0, "err" => "");

            $query = $this->db->query("SELECT lead_id, loan_noc_settlement_letter FROM loan WHERE loan_noc_settlement_letter IS NOT NULL AND loan_noc_settlement_letter !='' AND lead_id=$lead_id");
    ...
    public function generateRepayLinkMail($lead_id) {
        ...
        $user_id = $this->encrypt->encode($_SESSION['isUserSession']['user_id']);

        if (!empty($lead_id)) {
            $data = $this->Tasks->generate_Repay_Link_Mail($this->encrypt->decode($lead_id), $user_id);
            if ($data == "false") {
                $json['err'] = json_encode('Mail Failed ');
                echo json_encode($json);
            } else {
                $json['msg'] = json_encode('Repayment Request Link.');
                echo json_encode($json);
            }
        } else {
            $json['err'] = 'lead Id is Required.';
