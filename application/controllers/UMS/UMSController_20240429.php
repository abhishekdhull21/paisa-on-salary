...
//            $this->form_validation->set_rules('user_name', 'Login User Name', 'required|min_length[10]|max_length[50]');
    ...
    public function umsEditUser($enc_user_id = "") {
        ...
        if (!empty($enc_user_id)) {
            ...
            $user_id = intval($this->encrypt->encode($enc_user_id));
            ...
            if ($return_array['status'] == 1) {
                ...
                if ($this->input->server('REQUEST_METHOD') == 'POST') {
                    ...
                    $user_data["user_name"] = strval($this->input->post('user_name'));
                    $user_data["user_dialer_id"] = strval($this->input->post('user_dialer_id'));
                    $user_data["user_dialer_campaign_id"] = strval($this->input->post('user_dialer_campaign_id'));
                    $user_data["user_dialer_queue_id"] = strval($this->input->post('user_dialer_queue_id'));
                    $user_data["user_dialer_call_list_id"] = strval($this->input->post('user_dialer_call_list_id'));
                    $user_data["user_status_id"] = intval($this->input->post('user_status_id'));
...
//                    $this->form_validation->set_rules('user_name', 'Login User Name', 'required|min_length[10]|max_length[50]');
                    ...
                    if ($this->form_validation->run() == FALSE) {
                        ...
                        if ($this->umsModel->getCheckLoginUsername($user_data["user_name"], $user_id)) {
                            ...
                            $loginUserName = $user_data["user_name"];
                            $dialerID = !empty($user_data["user_dialer_id"]) ? $user_data["user_dialer_id"] : NULL;
                            $dialer_campaign_id = !empty($user_data["user_dialer_campaign_id"]) ? $user_data["user_dialer_campaign_id"] : NULL;
                            $dialer_queue_id = !empty($user_data["user_dialer_queue_id"]) ? $user_data["user_dialer_queue_id"] : NULL;
                            $dialer_call_list_id = !empty($user_data["user_dialer_call_list_id"]) ? $user_data["user_dialer_call_list_id"] : NULL;
                            ...
                            $insert_user_array["email"] = strtoupper($email);
                            $insert_user_array["mobile"] = $mobile;
                            $insert_user_array["user_dialer_campaign_id"] = $dialer_campaign_id;
                            $insert_user_array["user_dialer_queue_id"] = $dialer_queue_id;
                            $insert_user_array["user_dialer_call_list_id"] = $dialer_call_list_id;
                            $insert_user_array["user_dialer_id"] = !empty($dialerID) ? $dialerID : NULL;
                            $insert_user_array["ip"] = getIpAddress();
                            $insert_user_array["user_status_id"] = $user_data["user_status_id"];
    ...
    public function umsAddUserRole($enc_user_id) {
       ...
       // $view_data['master_collection_agency'] = $this->umsModel->getMasterCollectionAgencyList();
    ...
    public function umsAddUsersRole($enc_user_id) {
        ...
        if (!empty($enc_user_id)) {
                ...
                $return_array = $this->umsModel->getUmsUserDetails($enc_user_id);
...
;
        ...
        // if ($this->input->server('REQUEST_METHOD') == 'POST') {
           ...
           // $user_role_collection_agency_id = intval($this->input->post('user_role_agency_id'));
           ...
           // $this->form_validation->set_rules('user_role_scm_id', 'User Role SCM Id', 'numeric|trim');
            // if($user_data["user_role_type_id"] == 19) {
            //     $this->form_validation->set_rules('user_role_agency_id', 'User Role Agency Id', 'required|numeric|trim');
            // }
            ...
            if ($this->form_validation->run() == FALSE) {
              ...
              //  $rolestateID = !empty($user_role_locations_data["user_role_state_id"]) ? $user_role_locations_data["user_role_state_id"] : NULL;
                ...
                // if (empty($user_data["user_role_type_id"])) {
                ...
                if (!empty($roletypeID)) {
                    ...
                    //$insert_user_array["user_role_collection_agency_id"] = $user_role_collection_agency_id;
    ...
    public function umsEditUserRole($enc_user_id) {
        ...
        if (!empty($enc_user_id)) {
            ...
            if (!empty($user_role_id)) {
                ...
                if ($return_array['status'] == 1) {
                    ...
                    $user_role_agency_id = $return_array['user_role_data']['user_role_collection_agency_id'];
                } else {
                    $errors_msg = "User details not found.";
                }
            } else {
        ...
        //echo '<pre>';print_r($view_data['mis_master_list']);die;
        ...
        $view_data['getCreditHead'] = $this->umsModel->getCreditHeadList($user_id);
        $view_data['getMappedCreditHead'] = $this->umsModel->getMappedCreditHead($user_role_id);
        $view_data['getMappedRoleLevel'] = $this->umsModel->getMappedRoleLevel($user_role_id);
        $view_data['master_collection_agency'] = $this->umsModel->getMasterCollectionAgencyList();
        $view_data['user_role_agency_id'] = intval($user_role_agency_id);
...
//            $user_role_credit_head_id = intval($this->input->post('user_role_credit_head_id'));
            $reporting_id = intval($this->input->post('reporting_id'));
            ...
            if (isset($user_role_active) && $user_role_active_object->num_rows() > 0) {
                ...
                $update_user_role_active["user_role_collection_agency_id"] = $user_role_collection_agency_id;
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (!empty($scm_id)) {
                    ...
                    if (!empty($return_update_flag)) {
                        ...
                        return redirect(base_url('ums/view-user/' . $user_id), 'refresh');
                    } else {
