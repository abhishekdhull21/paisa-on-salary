...
//        $scm_user_lists = $this->Collection->scm_user_lists($conditions);
    ...
    public function umsEditUsersRole($enc_user_role_id) {
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            $user_role_credit_id = intval($this->input->post('user_role_credit_id'));
            ...
            if (!empty($user_role_level)) {
                ...
                if ($user_role_level == "L4") {
                    $update_user_role_active['user_role_supervisor_role_id'] = "";
                    $success_msg = "<b>" . $status . "</b> - User credit head role updated successfully.";
                }
                ...
                } else if (!empty($user_role_credit_id)) {
                    $update_user_role_active['user_role_supervisor_role_id'] = $user_role_credit_id;
                    $update_user_role_status = $this->umsModel->update('user_roles', ['user_role_id' => $user_role_id], $update_user_role_active);
    ...
    public function getCreditHeadRoles(){
        $view_data = array();
        $success_msg = "";
        $errors_msg = "";
        $update_flag = true;
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $user_id = intval($this->encrypt->decode($this->input->post('user_id')));
                $getdata = $this->umsModel->getCreditHeadL4List($user_id);
                $data['msg']=$getdata;
            }
        }
