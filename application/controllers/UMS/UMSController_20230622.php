...
//                    $this->form_validation->set_rules('user_name', 'Login User Name', 'required|min_length[10]|max_length[50]');
    ...
    public function umsEditUserRole($enc_user_id) {
        ...
        $view_data['getMappedCredit'] = $this->umsModel->getMappedCredit($user_role_id);
        $view_data['getCredit'] = $this->umsModel->getCreditList($user_id);
        ...
        $view_data['getMappedCreditHeadL4'] = $this->umsModel->getMappedCreditHeadL4($user_role_id);
//        $view_data['getCreditHeadL4'] = $this->umsModel->getCreditHeadL4List($user_id);
...
//        $user_role_credit_head_id = 0;
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            $user_export_id_array = intval($this->input->post('user_export_id'));
            $user_mis_id_array = intval($this->input->post('user_mis_id'));
            $user_role_branch_id_array = intval($this->input->post('user_role_branch_id'));
...
//            $user_role_credit_head_id = intval($this->input->post('user_role_credit_head_id'));
    ...
    public function getReportingList() {
        $return_array = array("status" => 0, "message" => "", "data" => array());

        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            $this->form_validation->set_rules('user_id', 'User Id', 'required');
            $this->form_validation->set_rules('level_scope', 'User Id', 'required');
            if ($this->form_validation->run() == FALSE) {
                $errors_msg = validation_errors();
                $return_array["message"] = $errors_msg;
            } else {
                $user_id = $this->input->post('user_id');
                $level_scope = $this->input->post('level_scope');
                $user_role_id = intval($this->encrypt->decode($user_role_id));
                $reporting_data = $this->umsModel->getReportingListModel($user_id, $level_scope);

                $return_array['status'] = 1;
                $return_array['data'] = $reporting_data;
            }
        } else {
            $return_array["message"] = "POST request failed.";
        }
