...
//        $scm_user_lists = $this->Collection->scm_user_lists($conditions);
    ...
    public function user_export_permission($user_export_data_array) {
        ...
        if (empty($user_export_data_array['user_id'])) {
            ...
            if (!empty($user_export_array)) {
                ...
                $this->umsModel->update('user_export_permission', ['export_permission_user_role_id' => $user_role_id], ['export_permission_active' => 1, 'export_permission_deleted' => 0], 'export_permission_export_id', $master_export_values);
    ...
    public function umsEditUsersRole($enc_user_role_id) {
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            $user_role_active_object = $this->db->select('user_role_active, user_role_deleted')->where($conditions)->from('user_roles')->get();
