    ...
    public function getVerificationDetails($leadID) {
        ...
        if (!empty($lead_id)) {
            ...
            if ($lead_details->num_rows() > 0) {
                ...
                $aadhaar_doc_details = $this->Tasks->select('docs_master_id in (1,2) AND docs_active = 1 AND docs_deleted = 0 AND lead_id=' . $lead_id, '*', 'docs');
...
// 
                ...
                if ($lead_customer->customer_digital_ekyc_flag == 1 && !empty($lead_customer->customer_ekyc_request_initiated_on) && !empty($lead_customer->customer_docs_available)) {
            ...
            if ($lead_details->num_rows() > 0) {
                ...
                if ($flag == "YES" && in_array($email_verification_type, array(1, 2))) {
                    ...
                    if ($email_verification_type == 2) {
                        ...
                        $api_response = payday_email_verification_api($method_name, $lead_id, $request_array);
                    }
