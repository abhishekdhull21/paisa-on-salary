...
//        $this->middlewareEmail($email_data['email'], $email_data['subject'], $email_data['message'], '', 99);
    ...
    public function screenerLeadAllocation() {
        ...
        $cif_pancard = $this->db->select('*')->where('cif_pancard', $panrepeatcardLD)->from('cif_customer')->get();
        $pancardCif = $cif_pancard->row();
        $pancardcifdt = $pancardCif->cif_pancard;
        ...
        //echo '<pre>';print_r($tempDetails);die; 
                    ...
                    if (!empty($tempDetails) && $tempDetails['status'] == 1) {
                        ...
                        foreach ($tempDetails as $user_data) {
                            ...
                            $master_user_lead[$i]['user_status_id'] = $user_data['user_status_id'];
                        ...
                        foreach ($master_user_lead as $user_key => $user_data) {
                            ...
                            $user_active_flag = $user_data['user_status_id'];
...
//        $email_return = $this->lead_allocation_email_notification("ajayl@salaryontime.com", $customer_name, $user_name, $user_mobile, $user_email);
    }
