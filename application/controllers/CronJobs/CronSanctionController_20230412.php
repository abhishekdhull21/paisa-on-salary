...
class CronSanctionController extends CI_Controller {
    ...
    var $cron_notification_email = 'shubham@surya.com';
...
//        $email_return = $this->lead_allocation_email_notification("shubham@surya.com", "Sushil Kumar", "Shubham Agrawal", 9560807913, 'sushil.kumar@surya.com');
...
//        $this->middlewareEmail($email_data['email'], $email_data['subject'], $email_data['message'], '', 99);
    ...
    public function screenerLeadAllocation() {
//        echo "<pre>";
        $start_datetime = date("d-m-Y H:i:s");
        $message = "";
        ...
        $tempDetails = $this->SanctionModel->get_users_lead_list();
        ...
        if (!empty($tempDetails) && $tempDetails['status'] == 1) {
            ...
            $tempDetails = $this->SanctionModel->get_lead_new_list();
            ...
            if (!empty($tempDetails) && $tempDetails['status'] == 1) {
                ...
                foreach ($tempDetails as $customer_data) {
                    ...
                    if ($customer_data['user_type'] == "NEW" && in_array(strtoupper($customer_data['utm_source']), array('LWREPEAT', 'REPEATNF', 'NFREPEATSMS', 'LWREPEATDB'))) {
                        $user_type = 2;
...
//        $email_return = $this->lead_allocation_email_notification("shubham.agrawal@loanwalle.com", $customer_name, $user_name, $user_mobile, $user_email);
    }
...
//        echo "<pre>";
        ...
        $cron_name = "notcontactableleadallocationcode";
...
//die;
            if (!empty($tempDetails) && $tempDetails['status'] == 1) {
                ...
                foreach ($tempDetails as $customer_data) {
                    ...
                    $mobile = $customer_data['mobile'];
                    $customer_name = ucwords(strtolower($customer_data['first_name']));
                    $lead_rejected_user_id = $customer_data['lead_rejected_user_id'];
...
//                    $user_type = 3;
                    ...
                    foreach ($master_user_lead as $user_key => $user_data) {
                        ...
                        $user_email = strtolower($user_data['email']);
//echo "<pre>";
//print_r($lead_rejected_assign_counter);
//continue;
//die;
                        //empty($user_active_flag) || empty($user_active_case_type) || ($user_type != $user_active_case_type)
