	...
	function get_bankAnalysis(lead_id,btn){
		...
		$.ajax({
            ...
            success: function (response) {
				...
				var res = JSON.parse(response);
				$(btn).css('display','none'); 
				var htmlTag = '<td colspan="3" ><strong>PDF: </strong> <a href="' + res.data.pdf + '" >Download</a><br/>';
...
//                            window.location.reload();
    ...
    function preAuditToggle(lead_id, audit_type) {
        ...
        var sendData = {lead_id: lead_id, audit_type: audit_type, csrf_token};
        $.ajax({
    ...
    function setSendLink(lead_id, customer_id, link_type)
    {
        ...
        var sendData = {lead_id: lead_id, customer_id: customer_id, link_type: link_type, csrf_token};
        $.ajax({
    ...
    function ResonForApprovalLoan()
    {
        ...
        if (lead_id == "") {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {user_id: user_id, lead_id: lead_id, comment: comment, audit: audit_id, csrf_token},
                dataType: 'json',
                beforeSend: function () {
    ...
    function ResonForRejectLoan()
    {
        ...
        if (lead_id == "") {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {user_id: user_id, lead_id: lead_id, customer_id: customer_id, reason: reason_id, reject_remark: reject_remark, audit: audit_id, csrf_token},
                dataType: 'json',
                beforeSend: function () {
...
<?php } ?>
                ...
                if ( res.lead_disbursal_approve_datetime) {
                    ...
                    html += '<tr><th class="thbg">Loan Disbursal Letter : </th><td><a onclick=download_disbursal_letter("' + res.lead_id + '") style="cursor:pointer;">Download</a> <br>Disbursed On : ' + ((res.disbursal_approve) ? res.disbursal_approve : '-') + '</td>';
                } else {
                    html += '<tr><th class="thbg">Loan Disbursal Letter </th><td> - </td>';
                }
                
                if (res.cam_sanction_letter_esgin_on) {
                    html += '<th class="thbg">Sanction Letter : </th><td><a onclick=download_sanction_letter("' + lead_id + '") style="cursor:pointer;">Download</a> <br>Sanction On : ' + ((res.cam_sanction_letter_esgin_on) ? res.cam_sanction_letter_esgin_on : '-') + '</td><th class="thbg"></th><td></td></tr>';
                } else {
    ...
    function download_noc_settlement_letter(lead_id) {

        {
            $.ajax({
                ...
                success: function (response) {
                    if (response.status) {
                        window.open("https://salaryontime.in/direct-document-file/" + response['file_name']);
...
//                        window.location.reload();
        ...
        if (pannumber.length == 10) {
            if (pannumber.match(regex)) {
                ...
                $.ajax({
                    ...
                    type: 'POST',
                    dataType: "json",
                    data: {pan_number: pannumber, action: action, csrf_token},
                    success: function (response) {
                        if (response.status == 1) {
                           ...
                           /*  $('#errorpancard').html(response.message).show().css({'color': 'green'});
                            ...
                            $('#errorpancard').html(response.message).show().css({'color': 'red'});
                            $('#datapancard').html(response.data.pancard);
                            $('#respone_data_number').html(response.respone_data.number);
                            $('#respone_data_name').html('Name: ' + response.respone_data.name);
                            $('#respone_data_father_name').html('Father Name: ' + response.respone_data.number);
    ...
    function ocr_verification_api_call(lead_id, ocr_verification_type) {
        ...
        $.ajax({
            ...
            type: 'POST',
            dataType: "json",
            data: {lead_id: lead_id, ocr_verification_type: ocr_verification_type, csrf_token},
            beforeSend: function () {
                $("#cover").show();
            },
            success: function (response) {
    ...
    function generateRepayLink(lead_id) {
        ...
        var url = "<?= WEBSITE_URL ?>repay-loan-details/" + lead_id + '/' + user_id + '';
        try
        {
...
//                        $('#repay_encrypted_url').html("<a href='" + response.repay_encrypted_url + "' target='_blank'>" + response.repay_encrypted_url + "</a>");
    ...
    function auditDetails(lead_id, user_id, audit)
    {
        if (lead_id != "") {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {'lead_id': lead_id, 'user_id': user_id, 'audit': audit, csrf_token},
                dataType: "json",
                success: function (response) {
                    if (response.errSession) {
                        window.location.href = "<?= base_url() ?>";
    ...
    function get_Customer_Feedback(lead_id)
    {
        ...
        } else
        {
            $.ajax({
                ...
                type: 'POST',
                data: {lead_id: lead_id, remarks: remarks, pancard: pancard, reloan_flag: reloan_flag, csrf_token},
                dataType: "json",
                success: function (response) {
                    catchSuccess(response.msg);
                    window.location.reload();
    ...
    if (lead_id == "") {
        ...
        $.ajax({
            url: '<?= base_url("send_settlement_loan_letter") ?>',
            type: 'POST',
            data: sendData,
