<?php
    ...
    //            ->row();
       ...
       // echo $url2; 
        $url_roles = $this->db->select('MURL.mau_id, MURL.mau_role_id, MURL.mau_url, MRT.role_type_id, MRT.role_type_labels')   
               ->where('MURL.mau_role_id', $_SESSION['isUserSession']['role_id'])
               ->where(['MURL.mau_active' => 1, 'MURL.mau_deleted' => 0])
               ->from('master_access_url MURL')
               ->join('master_role_type MRT', 'MURL.mau_role_id = MRT.role_type_id')
               ->get()
               ->result_array();
               $url_user_role = array();
               foreach($url_roles as $urlall){             
                   $url_user_role[] = $urlall['mau_url'];
               }
               if (in_array($url2, $url_user_role)) { }
                else if($url2=='home'){ }
                else if( $url2=='search') {}
                else if( $url2=='myProfile'){ }
                else if( $url2=='getleadDetails'){ }
                else if($_SESSION['isUserSession']['role_id']==1){}
               else{
                   return redirect(base_url());
                }
