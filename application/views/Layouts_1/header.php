...
if (empty($_SESSION['isUserSession']['user_id'])) {
    ...
    $user_roles = $this->db->select('UR.user_role_user_id, UR.user_role_type_id, MRT.role_type_labels,  MRT.role_type_id, MRT.role_type_name, MRT.role_type_heading')
    ...
    if(empty($_SESSION['isUserSession']['access_url'])) {

        $role_id = $_SESSION['isUserSession']['role_id'];

        $qry = "SELECT mau_role_id, mau_url FROM master_access_url WHERE mau_active=1 AND mau_deleted=0 AND mau_role_id=" . $role_id;

        $access_result = $this->db->query($qry)->result_array();

        $access_url_array = array();

        foreach ($access_result as $value) {
            $access_url_array[] = $value['mau_url'];
        }

        $_SESSION['isUserSession']['access_url'] = $access_url_array;
    }

    if((in_array($url1,$_SESSION['isUserSession']['access_url'])) || ($_SESSION['isUserSession']['role_id'] == 1) || ($url1 == 'home') || ($url1 == 'dashboard') || ($url1 == 'search') || ($url1 == 'getleadDetails') || ($url2 == 'edit-blog') || ($url2 == 'add-blog') || ($url1 == 'legal') || ($url1 == 'collection') || ($url1 == 'collection-pending') || ($url1 == 'recovery-pending') || ($url1 == 'settlement') || ($url1 == 'write-off') || ($url1 == 'visitrequest') || ($url1 == 'visitpending') || ($url1 == 'visitcompleted') || ($url1 == 'exportData') || ($url1 == 'MIS-Report')) {
        //continue;
    } else {
        return redirect(base_url());
    }
