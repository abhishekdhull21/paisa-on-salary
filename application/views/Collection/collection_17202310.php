...
<div id="Request_Field_Visit" class="collapse"> 
    ...
    <div id="AddCollectionVisit" class="collapse"> 
        <form id="FormRequestForCollectionVisit" class="form-inline" method="post" enctype="multipart/form-data" style="margin: 10px;">
            ...
            <?php if (in_array(agent, ['CO1']) && in_array($leadDetails->lead_status_id, [14, 19])) { ?>
                <div class="col-md-6" style="margin-bottom: 15px;">
...
<?php if (in_array(agent, ["CA", "CO3"])) { ?>
    ...
    <div>     
        <div id="LegalNotice" style="display: none;">           
            <!--   
             <form id="FormLegalNotice" class="form-inline" method="post" enctype="multipart/form-data" style="margin: 10px;">
                 <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>" />
                 <input type="hidden" name="lead_id" id="lead_id" value="<?= $this->encrypt->encode($leadDetails->lead_id) ?>" readonly="">
                 <input type="hidden" name="pancard" id="pancard_id" value="<?= $leadDetails->pancard ?>" readonly=""> 
                 <div class="col-md-12" style="margin-bottom: 15px;">
                     <label class="labelField">Name&nbsp;<strong class="required_Fields">*</strong> </label>
                     <input type="text" class="form-control" maxlength="50" id="customer_name" required name="customer_name" autocomplete="off">
                 </div>           
             </form>    
             <div class="col-md-12" style="margin: 10px;">
                 <div id="btn_save_Legal_notice">
                     <button id="saveLegalNotice" class="btn btn-success lead-Legal-notice-button" onclick="getLegalNotice('<?= $this->encrypt->encode($leadDetails->lead_id) ?>')">Update </button> 
                 </div>
             </div>
            -->
            <div style="background:#fff !important;">
                <p><b>Send Legal Notice</b>&nbsp;&nbsp;<input type="checkbox" name="send_email_legal_notice" id="send_email_legal_notice" value="1" onclick="sendLegalNotice('<?= $leadDetails->lead_id ?>', '<?= $leadDetails->customer_id ?>', 1)">&nbsp;<label for="send_email_legal_notice"><b>Send Email</b></label>&nbsp;&nbsp;&nbsp;<input type="checkbox" name="send_sms_legal_notice" id="send_sms_legal_notice" value="1" onclick="sendLegalNotice('<?= $leadDetails->lead_id ?>', '<?= $leadDetails->customer_id ?>', 2)">&nbsp;<label for="send_sms_legal_notice"><b>Send SMS</b></label>&nbsp;&nbsp;
                    <a href="<?= base_url('view-legal-notice/' . $this->encrypt->encode($leadDetails->lead_id)); ?>" title="View" target="_blank"><i class="fa fa-eye" style="padding : 3px; color : #35b7c4; border : 1px solid #35b7c4;"></i></a>                            
                </p>
            </div>     
        </div>
    </div>
